...
1. 检查 `members/main/lock`：
   - **无 main**（无 lock / PID 已死）→ 成为 **main 协调者**：
     ...
     3. 用 `--append-system-prompt` 注入 main 协调者指令（扮演 main：读 `roles/main.md`、按 L4-L7 等级路由拆任务、投递到 `tasks/pending/`、验收时扫描已验收/打回），并加 `--permission-mode bypassPermissions` 全自动干活
