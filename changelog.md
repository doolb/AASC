...
- ✅ [2026-09-23] 支持从角色身上拖动旋转，只有轻点才触发角色触摸。
  - `display-mmd.js` 为角色命中和空白起点统一建立旋转拖动状态，移动达到 8 像素即取消触摸候选；拖动完成或取消不发布 `mmd.interaction`，角色轻点松开后仍执行原有互动。
  - 更新显示端聊天/MMD design、spec、task、todo、usage、自测和指针事件回归；定向测试 56/56、JavaScript 语法及 `git diff --check` 通过。完整 `npm test` 为 928/985 通过，57 项其他模块或本机环境用例失败；真实 PMX/VRM 触屏待现场验收，服务代码需重新出包。
...
- ✅ [2026-09-23] 增加“停一下”停止当前语音播报的命令别名。
  - 命令注册表和处理入口复用 `stopTts`；动态帮助展示该短语，停止目标路由和“已停止播报”确认保持不变。
  - 更新语音会话 design/spec 与使用说明；未新增或运行自动化测试，未打包。
...
- ✅ [2026-09-23] 外网 Offline 同步显示文件名和下载进度
  ...
  - 更新同步 design/spec、使用说明和任务记录；Node 语法检查与 `git diff --check` 通过，未运行自动化测试或构建 Offline APK/发布包。
- ✅ [2026-09-23] 修复 Offline 同步重复下载已存在资源
  - 下载前检查目标文件大小和 SHA-256；匹配时跳过 HTTP 下载并显示 `[已复用]`，冲突则在传输前失败；保留安装阶段二次检查。
  - 下载进度行使用 `[下载进度]` 标识；更新同步 design/spec、使用说明和任务记录。Node 语法检查与 `git diff --check` 通过，未运行自动化测试或构建 Offline APK/发布包。
...
- ✅ [2026-09-22] 将 PMX/VRM 拖动旋转改为模型中心枢轴与指数缓动。
  - 归一化且贴地的模型保留自身动画/命中网格，新增父级中心枢轴负责用户的转身与俯仰；目标角度以约 140ms 时间常数收敛。
  - 指针松开后只在角度收敛时按最终姿态刷新阴影相机，避免脚底俯仰和阴影跳动。
  - 验证：`node --test tests/display-chat-mmd.test.js` 34/34、PMX/VRM runtime 语法检查与 `git diff --check` 通过。
...
- ✅ [2026-09-22] 增加显示端右上角“灯光”按钮和详细灯光面板。
  - 支持环境光/主方向光颜色、强度和主光经度/纬度方向调整，提供默认、柔和、明亮预设及“恢复默认”；光源距离固定。
...
- ✅ [2026-09-23] 角色灯光主方向由 X/Y/Z 改为经度/纬度。
  - 灯光面板只显示经度 `-180°..180°` 和纬度 `-90°..90°`；PMX/VRM runtime 使用固定光源距离转换为 Three.js 坐标。
  - 旧版 localStorage 的 `keyPosition` 会自动迁移为 `keyDirection`，规范化输出不再包含 X/Y/Z；验证：显示端测试 39/39、四份灯光相关脚本语法检查和 `git diff --check` 通过。
...
- ✅ [2026-09-21] 修复 Android 高 DPI/300% 缩放下 TTS 播报文字超出屏幕、竖屏左右留白过大的问题。
  - `display.html` 按旋转后的逻辑画布短边、可用宽度和文字长度动态计算字号，范围为 `16px–36px`；动态缩小 90/270 度竖屏侧边距。
  - `display.css` 增加自适应基础字号、最大高度、自动换行和纵向滚动；不改变 TTS 音频、聊天隐藏和旋转层级。
  - 增加 300% 手机视口包围盒回归测试；显示端相关测试 `32/32` 通过，`git diff --check` 通过。
  - 已发布服务代码 code v24：`15,188,989` bytes，SHA-256 `56dcc19b95658d551c5988b62507cf0b38f0416dc532e8923901445044253c4a`；沿用 dependencies v5，未重新发布 APK 或依赖包。
  - LAN/WAN 清单验签、HTTP `HEAD` 大小、远端 SHA-256 均已校验，旧 code v23 已按版本规则精确清理。

### 天气播报按显示端语音能力分流

- ✅ [2026-09-21] 修复有 `voicePlayback` 能力的来源端同时显示 TTS 播报文字和天气详情弹窗的问题。
  - 服务端天气结果复用来源显示端能力判断：有能力时只保留通用 TTS，只有无能力时才发送 `weatherResult` 天气详情文字兜底。
  - 天气详情仍位于 MMD 上方、聊天层下方；聊天打开时隐藏，关闭后按原弹窗生命周期恢复；不改变实际 TTS 目标选择。
  - 天气、显示端聊天和语音路由定向测试 45/45 通过，服务端/天气模块语法检查和 `git diff --check` 通过。
  - 已发布服务代码 code v23：`15,180,480` bytes，SHA-256 `98c5484c9ac234e8d84378fd730a06dea8ffe34fd44177d6bab510fcee9615dc`；沿用 dependencies v5，未重新发布 APK 或依赖包。
  - LAN/WAN 清单验签、HTTP `HEAD` 大小、远端 SHA-256 均已校验，旧 code v22 已按版本规则精确清理。

### 普通聊天回复按显示端语音能力分流

- ✅ [2026-09-21] 修复有语音播放能力的显示端同时出现 TTS 播报和 response 弹窗的问题。
  - 服务端复用来源显示端 `voicePlayback` 能力判断：有能力时只发送 TTS 音频/播报文字和聊天同步消息；无能力时继续发送 response 文字兜底。
  - 不改变无声显示端文字兜底，也不改变来源端无能力时由其他显示端实际播放 TTS 的路由。
  - 定向显示端/普通语音聊天测试 32/32 通过。
  - 已发布服务代码 code v22：`15,180,288` bytes，SHA-256 `fad58da51e3ef35dabda6edc4d63cbdee8422a9e8dc9e630c7cb2ece4a90921a`；沿用 dependencies v5，未重新发布 APK 或依赖包。
  - LAN/WAN 签名清单、HTTP `HEAD` 大小、远端 SHA-256 均已校验，旧 code v21 已按版本规则精确清理。
...
- ✅ [2026-09-20] 调整纯助手名唤醒临时模式的随机确认文案。
  - 将候选回复“哎，我在”替换为“啊，我在”；其他候选回复、带内容确认和临时模式处理流程保持不变。
  - 同步更新 `server-app.js`、临时语音会话 design/spec/task/todo 文档。
...
- ✅ [2026-09-19] 增强临时语音会话历史和群聊控制指令。
  - 临时会话按连续会话组保存，默认保留最近 10 组；模型上下文发送当前组和最近 2 组，共 3 组，两个数量均可通过控制端配置。
  - 支持 `小爱再见`/`再见小爱` 退出临时模式；支持 `进入群聊` 免唤醒进入持续群聊、`退出群聊` 结束持续群聊。
  - 未指定日期的 `天气`/`北京天气` 播报今天、明天、后天；明确日期时仅播报目标日期；临时唤醒提示本次保持不变。
  - 改动文件：临时会话状态机、历史选择模块、服务端 LLM 上下文、语音配置面板、控制端命令帮助及对应设计/spec/task 文档。
  - 验证：定向语音/天气/历史/配置测试通过，Chat2API 92/92，相关语法检查和 `git diff --check` 通过；全量测试剩余 1 个既有 Offline 固定 displayId 基线失败。

### 控制端历史临时会话

- ✅ [2026-09-19] 增加历史临时会话只读查看。
...
- ✅ [2026-09-25] 历史临时会话切换改为下拉选择。
  - 下拉选项包含开始时间、角色和消息数量；保留只读查看及“当前对话”返回入口，窄屏时选择器可收缩。
  - 更新聊天设计/spec、任务文档和手动验收清单。语法与差异检查通过；自动化测试未运行，页面现场操作待验收。
...
- ✅ [2026-09-07] 修复主控制端播放子服务器媒体时直连 URL 主机不正确及批量播放列表地址缺失的问题。
  - 远程媒体索引和远程播放列表统一使用 AASC 注册表中的子服务器 `node.url` 作为直连地址主机，保留原始媒体路径。
  - 播放列表保留文件 `path`，主服务器同时生成远程代理 `fallbackUrl`；显示端直连失败时回退主服务器代理，直连和代理都为空时不发起媒体请求。
  - 改动文件：`src/framework/aasc/media-index-service.js`、`src/apps/server/boot/server-app.js`、播放列表服务、控制端媒体库、显示端页面、相关测试及 AASC 文档。
  - 同时修复媒体库目录项被误判为无地址后无法进入子目录的问题。
  - 现场强制更新 Termux 子服务器后，`/storage/dcim/11月26日花火_720p.mp4` 聚合为 `mediaType=video`；子服务器直连和主服务器代理均返回 HTTP 200、`video/mp4`。
  - 验证：媒体索引 6/6、媒体库 UI 6/6、播放列表 13/13、服务器契约 8/8、播放恢复 5/5 通过；JavaScript 语法检查通过。
...
- ✅ [2026-09-02] 补充真正单大核/单小核五模型速度基线，纠正与核心集合模式的表述混淆。
  - 测试条件：同一 Android 9 arm64 真机、`bus.jpg`、五模型串行，预热 1 次、正式 2 次；单核心模式配置 `intraOp=1`、`interOp=1`。
  - 单大核 n/s/m/l/x 为 `1368.0/3997.0/12190.0/15533.0/34630.0 ms`，FPS 为 `0.731/0.250/0.082/0.064/0.029`。
  - 单小核 n/s/m/l/x 为 `1945.0/5530.5/16874.0/21607.0/46533.5 ms`，FPS 为 `0.514/0.181/0.059/0.046/0.021`；五模型一轮正式样本合计单大核 `67.718 s`、单小核 `92.490 s`。
...
- ✅ [2026-09-01] 新增独立 Android YOLO11 五模型 HTTP/网页测试 APK。
  ...
  - 准确性记录：Android 9 arm64 真机使用 `bus.jpg` 定性验证，检测到 1 辆公交车和 4 个人，置信度为公交车 `0.939`、人员 `0.902/0.849/0.833/0.396`；未使用标注集计算正式 Precision、Recall 或 mAP。
...
- 完成任务从 `docs/todo.md` 移除，完成摘要、改动和验证结果归档到 `changelog.md`，避免已完成条目长期留在待办清单中。
- 更新 `AGENTS.md`、`docs/design/ai-rules.md` 和 `docs/spec/ai-rules.md`，统一工作 AI 与 workgroup 的文档维护规则。
...
- 控制端新增独立搜索频道，显示搜索中、完成、失败状态和搜索历史，不再将搜索请求作为群聊消息展示。
- 修复控制端收到 `searchHistory` 后未调用刷新导致历史不可见的问题。
- LLM/Pi 搜索只发送当前搜索请求，每次创建一次性 Pi RPC 进程，完成或失败后自动回收，避免继承群聊上下文和累积进程。
- 搜索命令开始时先播报 `正在搜索${内容}`，system 与 LLM/Pi 搜索结果统一记录到搜索历史。
- 测试：Pi Runtime、LLM Agent、搜索频道、语音和 Markdown 控制端回归 47/47 通过。

### 系统搜索单次内置任务

- 系统搜索注册为可手动调用的 `search.web` 单次内置任务，使用 Axios 请求 Bing、Cheerio 解析前十条真实网页结果。
- 兼容中文/英文绝对日期和中英文相对日期，按日期排序；全部无日期时保持 Bing 原顺序。
- 任务按日期排序后返回前五条；搜索频道、历史和弹窗展示五条，TTS 播报前三条。
- 搜索/展示/TTS 数量保存为 `search.web` 任务级全局配置，无需创建实例即可设置，默认值为 10/5/3。
- 语音搜索复用任务引擎，任务只返回结果，TTS、搜索频道和历史由外层流程处理；不启动 Chromium 或 Pi 进程。
- 控制端可从内置任务列表输入 `query` 手动执行，并通过标准 `task:result` 获取结果。
- 测试：搜索任务、任务引擎、搜索频道和显示端语音相关回归全部通过。

### 任务 Widget 刷新日志降噪

- 静默运行中任务周期性 `widgetRefresh` 的控制端收发日志，以及 `task:widget_update` 广播日志。
- 保持 widget 刷新、数据推送和人工 `updateConfig` 操作不变，异常与其他业务日志继续记录。
- 测试：widget 日志降噪专项 2/2 通过。

### 任务列表轮询日志降噪

- 静默正常 `task:list` 的控制端收发日志，保留任务列表处理失败日志。
- 保持任务列表查询、任务结果和控制端刷新行为不变。
- 测试：任务列表/Widget 日志降噪专项 3/3 通过。

### 等待唤醒确认词误匹配修复

- 修复确认/拒绝/取消内置指令使用包含匹配导致的误放行；“拒绝奖励出街注意”等普通语音不再在等待唤醒状态下进入命令处理链路。
- 保留“拒绝”“取消”“确认添加”等完整短指令的免唤醒执行能力。
- 测试：内置指令和显示端语音会话回归 29/29 通过。
- 详细任务：`docs/task/2026-08-30_等待唤醒确认词误匹配.md`。
...
- 🔄 [2026-08-30] 增加原 Chat2API 数据一键迁移
  - 控制端账户管理弹窗增加“导入原 Chat2API 数据”按钮，服务端读取同用户 `~/.chat2api/data.json`。
  - 迁移前返回脱敏预览，确认后合并 Provider、账号、模型映射和代理基础配置，不删除现有 AASC 数据。
  - 原 Chat2API API Key、日志、会话和 Electron 桌面状态不迁移；账号凭据仅写入 AASC 私有数据目录。
  - 测试：Chat2API 数据层、管理服务、代理路由和控制端 UI 回归通过。
  - 详细任务：`docs/task/2026-08-30_Chat2API核心内置任务.md`。

- 🔄 [2026-08-30] 修复 Chat2API 控制端 HTTPS 页面无法导入
  - 增加 AASC 主服务同源 HTTPS 网关，按 `chat2api.proxy` 任务实例转发到服务器本机代理，避免浏览器混合内容拦截和远程控制端误用 `127.0.0.1`。
  - 任务参数兼容字符串形式的 `false/true`，确保 API Key 开关与控制端选择一致。
  - 测试：同源网关、任务参数和控制端回归通过。

- ✅ [2026-08-30] 修复 Chat2API 账户管理页面主题适配
  - 账户管理弹窗、配置表单、账号/API Key/模型映射列表移除固定深色内联样式，统一使用 `chat2api.css` 和控制端主题变量。
  - 增加窄屏单列布局，主题切换后弹窗表面、文字、输入控件、边框和语义状态色同步更新。
  - 测试：控制端 Chat2API UI 主题契约和完整 Chat2API 回归通过。

- ✅ [2026-08-30] 增加 Chat2API 模型映射手动编辑
  - 控制端账户管理页面支持模型映射新增、编辑、删除，并可指定优先 Provider 与账号。
  - 修改请求模型时先保存新映射再删除旧键，避免产生残留映射。
  - 测试：控制端 UI 契约和完整 Chat2API 回归通过。

- ✅ [2026-08-30] 修复原 Chat2API 用户模型映射未迁移
  - 补充读取 `userModelOverrides.*.addedModels`，将用户添加的模型别名转换为 AASC 模型映射并保留 Provider 归属。
  - 原配置中的 `Qwen3.6-Flash → Qwen3.7` 现在会在迁移预览中显示并完成导入。
  - 测试：数据迁移测试和完整 Chat2API 回归通过。

- ✅ [2026-08-30] 修复 Chat2API 用户模型映射无可用账号
  - 模型解析兼容历史 `providerId` 字段，并把指定 Provider 作为自定义模型的路由约束。
  - `Qwen3.6-Flash` 可通过 qwen 活动账号选择，最终按映射转发为 `Qwen3.7`。
  - 测试：模型映射、负载均衡和完整 Chat2API 回归通过。

- ✅ [2026-08-30] 修复 Qwen 模型返回空内容
  - 增加 Qwen 专用响应适配，解析 `data.messages` 中的 `multi_load/iframe`、`text/plain` 内容。
  - 非流式请求聚合 SSE 返回，流式请求处理累计文本并过滤思考标记，避免只返回 `[DONE]` 或空内容。
  - 测试：Provider adapter 非流式/流式解析和完整 Chat2API 回归通过。
...
- ✅ [2026-08-30] 添加 Chat2API 到 Pi 的工具转换
  ...
  - 验证：新增及现有 Agent/聊天测试 31/31，真实 Pi 0.84.2 假 SSE 成功执行 `read`，语法检查和 `git diff --check` 通过。
  - 文档：`docs/design/llm-agent-mode.md`、`docs/spec/llm-agent-mode.md`、`docs/task/2026-08-30_添加Chat2API到Pi工具转换.md`。

- ✅ [2026-08-30] 系统帮助弹窗提前显示
  - 完整帮助文本先发送到目标显示端立即打开语音响应弹窗，分句 TTS 在后台按批次生成播放，避免弹窗等待全部音频生成。
  - 验证：系统帮助、批次完成状态、显示端语音会话、普通聊天和 TTS 路由相关回归 17/17 通过，服务端脚本语法检查通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-29_系统指令动态播报全部命令.md`。

- ✅ [2026-08-30] 修复分句 TTS 提前结束语音会话
  - 系统帮助分句共享 TTS 批次标识，显示端只有最后一句真正播放完成后才发送 `voiceConversationTtsFinished`，避免分句生成间隔造成提前恢复监听。
  - `tts action=stop` 会清空当前句、后续句子和批次状态，不误报整段播报完成。
  - 验证：系统帮助分句、批次完成状态、显示端语音会话、普通聊天和 TTS 路由相关回归 17/17 通过，服务端脚本语法检查通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-29_系统指令动态播报全部命令.md`。

- ✅ [2026-08-30] 系统帮助 TTS 按句分段生成播放
  - 系统帮助复用现有句子切分和串行调度逻辑，逐句生成并按顺序发送，避免整段帮助文本一次性生成超长音频。
  - 显示端通用播放和控制端定向播放目标保持不变，控制端帮助弹窗仍展示完整帮助文本。
  - 验证：内置语音指令、显示端语音会话、普通聊天和 TTS 路由相关回归 16/16 通过，服务端脚本语法检查通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-29_系统指令动态播报全部命令.md`。
...
- ✅ [2026-08-30] 修复多显示端与多来源 render-display 链接冲突
  ...
  - 验证：任务链接、面板和任务恢复回归 15/15 通过。
  - 文档：`docs/design/monitor-system.md`、`docs/spec/monitor-system.md`、`docs/task/2026-08-30_任务链接面板统一管理.md`。

- ✅ [2026-08-30] 选择目标后即时刷新任务链接列表
  - 选择目标后立即从添加列表移动到已链接列表，按钮切换为“解除”，弹窗保持打开，支持连续管理多个目标。
  - 验证：任务链接、链接面板和任务恢复回归 12/12 通过。
  - 文档：`docs/design/monitor-system.md`、`docs/spec/monitor-system.md`、`docs/task/2026-08-30_任务链接面板统一管理.md`。

- ✅ [2026-08-30] 缩小显示端语音柱状图并稳定识别文本定位
  - 音频柱状图宽度由 280px 缩小为 140px，监听状态与柱状图仍保持不换行整体居中。
  - 识别文本使用独立固定边缘锚点，避免状态行尺寸变化导致文本位置跳动。
  - 验证：显示端旋转、语音 UI 和监听回归通过，页面内联脚本编译和 `git diff --check` 通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-30_显示端语音状态旋转与监视图动画.md`。

- ✅ [2026-08-30] 修复旋转后语音文字重复排版导致的位置跳动
  - 语音状态行和识别文字按当前实际尺寸直接计算旋转边缘中心锚点，避免重复排版叠加旧补偿值。
  - 文本更新时不再重新初始化整套语音 UI，首次显示的识别文字在下一帧完成锚点稳定。
  - 验证：显示端旋转回归 9/9，语音 UI、监听、会话、TTS 路由和资源回归共 14/14 通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-30_显示端语音状态旋转与监视图动画.md`。

- ✅ [2026-08-30] 选择任务链接目标后保持面板打开
  - 点击“选择”发送链接请求后不关闭链接弹窗，支持连续选择多个目标显示端。
  - 验证：任务链接、链接面板和任务恢复回归 12/12 通过。
  - 文档：`docs/design/monitor-system.md`、`docs/spec/monitor-system.md`、`docs/task/2026-08-30_任务链接面板统一管理.md`。

- ✅ [2026-08-30] 解除任务链接后将运行中的目标移到添加列表
  - 解除后仍运行的目标立即从已链接列表移动到添加列表，弹窗保持打开并可立即重新链接；已停止目标只移除。
  - 验证：任务链接与链接面板回归 6/6 通过。
  - 文档：`docs/design/monitor-system.md`、`docs/spec/monitor-system.md`、`docs/task/2026-08-30_任务链接面板统一管理.md`。

- ✅ [2026-08-30] 修复任务链接面板单项解除误解除全部目标
  - 已链接目标节点保存目标实例 ID，解除操作只针对当前目标；服务端拒绝缺少目标 ID 的请求，避免误解除来源的全部链接。
  - 验证：任务链接与链接面板回归 6/6 通过。
  - 文档：`docs/spec/monitor-system.md`、`docs/task/2026-08-30_任务链接面板统一管理.md`。

- ✅ [2026-08-30] 调整显示端监听状态与柱状图为整体居中单行
  - 监听状态与 280px 音频柱状图组成不换行的横向状态行，整体在顶部语音区域居中；最近识别文本放在下一行。
  - 保持 0/90/180/270 度旋转适配，并限制监听状态宽度，避免长文本撑破状态行。
  - 验证：显示端旋转回归 9/9、语音聚焦回归 14/14，页面内联脚本编译和 `git diff --check` 通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-30_显示端语音状态旋转与监视图动画.md`。

- ✅ [2026-08-30] 调整显示端语音状态颜色与顶部音频柱状图
  - 监听中使用明显绿色，暂停监听使用明显橙色并分别保留状态动画。
  - 音频柱状图移动到语音状态后方顶部居中，宽度调整为 280px，最近识别文本排在其后并继续适配四向旋转。
  - 验证：显示端语音/旋转聚焦回归 14/14 通过，页面内联脚本编译和 `git diff --check` 通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-30_显示端语音状态旋转与监视图动画.md`。

- ✅ [2026-08-30] 统一任务链接面板管理多个显示端
  - 一个来源实例支持绑定多个目标实例，重复绑定幂等，任务列表返回过滤失效记录后的有效链接。
  - 复用现有“链接到...”弹窗，增加已链接目标、目标显示端、状态和解除操作，链接变更后自动刷新。
  - 验证：任务链接、任务面板、任务恢复和渲染更新相关回归 13/13 通过；脚本语法检查和 `git diff --check` 通过。
  - 文档：`docs/design/monitor-system.md`、`docs/spec/monitor-system.md`、`docs/task/2026-08-30_任务链接面板统一管理.md`。

- ✅ [2026-08-30] 限制天气弹窗最长显示 90 秒
  - 天气详情弹窗按每 3 个可见字符 1 秒计算，限制在 30～90 秒，避免超长详情长期占用显示端。
  - 普通语音响应默认 5 秒、天气 TTS 和结构化天气数据保持不变。
  - 验证：天气弹窗契约断言通过；完整旋转测试的其他失败与本次无关。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-30_天气弹窗最长90秒.md`。

- ✅ [2026-08-30] 调整显示端语音 UI 到顶部居中
  - 语音状态和最近识别文本改为逻辑顶部边缘居中上下排列；旋转后分别映射到对应边缘中心，长文本继续受逻辑画布限制。
  - 语音 UI 保持四向旋转适配，音频监视图继续使用独立角落布局。
  - 验证：显示端语音 UI 契约测试通过，内联脚本编译通过，`git diff --check` 通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-30_显示端语音状态旋转与监视图动画.md`。

- ✅ [2026-08-30] 修复显示端语音状态、旋转适配和音频监视图动画
  - 语音状态明确显示“未就绪”“监听中”“暂停监听”，TTS 暂停期间复用既有状态并正确反馈。
  - 语音状态和音频监视图统一跟随 0/90/180/270 度布局；监视图复用频谱缓冲并保持低信号动态反馈，减少每帧内存分配。
  - 验证：页面内联脚本编译通过，显示端语音/旋转聚焦回归 27/27 通过，`git diff --check` 通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-30_显示端语音状态旋转与监视图动画.md`。

- ✅ [2026-08-30] 天气详情逐时数据只保留今日
  - 天气弹窗详情仅展开第一天预报的逐时天气，未来日期保留逐日汇总和天文信息，不展开逐时明细。
  - 结构化天气对象继续保留完整逐时数组，TTS 播报范围保持不变。
  - 验证：天气专项测试 6/6 通过。
  - 文档：`docs/design/display-control-refactor.md`、`docs/design/display-voice-conversation.md`、`docs/spec/voiceCommand.md`、`docs/task/2026-08-30_天气详情逐时仅今日.md`。

- ✅ [2026-08-30] 修复服务器重启后显示端监听不自动恢复
  - 显示端每次 WebSocket 重连后重新检查服务端 ASR，ASR 暂未就绪或请求失败时每 5 秒重试，恢复后自动启动持续监听。
  - WebSocket `error` 事件主动释放异常连接并进入单飞重连流程；页面离开和关闭监听时清理相关定时器。
  - 验证：重连与语音相关聚焦测试 14/14、`git diff --check` 通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-30_服务器重启后显示端监听自动恢复.md`。

- ✅ [2026-08-30] 修复重复报时期间监听提前恢复
  - 整点报时三次重复播放复用同一 TTS 播放组，服务端等待全部重复次数完成后才广播结束状态。
  - Node 子显示端一次回报本地队列完成数量；普通单次 TTS 继续按一次完成处理。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-30_修复重复报时监听提前恢复.md`。

- ✅ [2026-08-30] TTS 跨显示端播报期间暂停无声纹录音
  - 服务端为实际 TTS 音频注入播放 ID，向所有录音显示端广播播报状态；录音端等实际播放结束后再恢复。
  - Web 显示端和 Node 子显示端均复用既有暂停/恢复链路，支持多播报目标、连续句子、停止/失败和旧客户端超时兜底。
  - 服务端额外拦截播报期间已在途的无声纹 ASR 结果，避免播报回声触发语音指令。
  - 验证：跨端协议契约 1/1、语音相关回归 45/45、语法检查和 `git diff --check` 通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-30_跨显示端TTS期间暂停监听.md`。
...
- ✅ [2026-08-29] 系统指令动态播报全部可用命令
  ...
  - 验证：语音会话 6/6、内置指令 10/10、TTS 路由 3/3 通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-29_系统指令动态播报全部命令.md`。

- ✅ [2026-08-29] 显示端复用子显示端 VAD 分段缓存
  - 浏览器显示端 VAD 触发前不再把等待期间的大段静音写入 ASR WAV，检测到语音时并入约 300ms 前置缓冲，保留判停所需的尾部静音。
  - 仅显示端启用分段模式，控制端聊天和声纹注册继续使用完整 PCM；持续监听仍复用同一个 AudioContext 和音频采集链路。
  - 验证：显示端 ASR、持续监听、VAD、唤醒/TTS 和共享录音入口回归 15/15 通过，脚本语法检查和 `git diff --check` 通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-29_显示端复用子显示端VAD分段缓存.md`。

- ✅ [2026-08-29] VAD 卡片只显示当前选中的显示端
  - VAD 阈值、底噪检测和统计结果卡片改为单设备模式，随当前显示端选择自动刷新。
  - 未选择或设备离线时显示提示，原有监听、保存和检测协议不变。
  - 验证：VAD 布局、监听状态和相关脚本检查通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-29_显示端VAD当前设备卡片.md`。
...
- ✅ [2026-08-29] 增加显示端 VAD 设置与底噪检测
  ...
  - 检测期间只采样分析，不上传 ASR、不触发语音命令；显示端重连后恢复已保存阈值。
  - 验证：VAD 契约、显示端监听/会话/内置指令/TTS、服务端语音开关回归通过；脚本语法检查和 `git diff --check` 通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-29_显示端VAD配置与底噪检测.md`。

- ✅ [2026-08-29] 显示端语音输入 TTS 改走通用播放路由
  - 显示端语音命令、唤醒提示和语音触发的普通对话不再默认把音频发回输入显示端，统一按在线 `voicePlayback` 能力发送。
  - 语音命令服务移除底层 TTS 直接调用，控制端原有定向播放和 `playOnControl` 行为保持不变；停止播报同步按来源区分广播或定向停止。
  - 验证：显示端语音 TTS 路由契约、通用 TTS、唤醒状态、监听、ASR 和服务端语音开关回归通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-29_显示端语音输入TTS通用路由.md`。

- ✅ [2026-08-29] 控制端系统指令帮助动态显示内置命令列表
  - 服务端语音命令模块统一提供内置命令清单，控制端通过 WebSocket 加载并展示示例、功能说明和“无需唤醒”标记。
  - 初始化及 WebSocket 重连时自动刷新清单，保留原有自定义命令协议和语音执行流程。
  - 验证：命令列表契约测试 10/10、显示端语音状态 5/5、监听链路 27/27，前端脚本语法检查通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-29_控制端内置命令列表显示.md`。

- ✅ [2026-08-29] 显示端等待唤醒时支持内置功能指令免唤醒执行
  - “现在几点”“报时”“提醒”“静音”“天气”“搜索”“播放”等现有内置功能不再要求先说唤醒词。
  - 内置指令执行后仍保持 `waitingWake`；普通聊天、自定义命令和会话控制词仍需唤醒词。
  - 验证：状态机 5/5、内置分类 5/5、监听链路 27/27，相关显示列表、ASR、TTS 和服务端语音开关回归 12/12 通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-29_显示端等待唤醒内置指令直通.md`。
...
- ✅ [2026-08-29] 降低浅色主题大卡片背景亮度
  - 大卡片 `.section` 改用 `bg-secondary`，避免浅色主题大面积近白；小卡片和弹窗颜色语义保持不变。
  - 文档：`docs/design/control-ui-theme.md`、`docs/spec/ui-theme.md`、`docs/task/2026-08-29_浅色主题大卡片背景降低亮度.md`。

- ✅ [2026-08-29] 修复浅色主题大小卡片背景层级
  - `.section` 大卡片改用 `bg-surface`，`.control-item` 小卡片继续使用 `card-background`，解决两者颜色相同、视觉层级分不开的问题。
  - 任务卡片、媒体条目和弹窗继续使用原有主题变量；深色主题不变。
  - 文档：`docs/design/control-ui-theme.md`、`docs/spec/ui-theme.md`、`docs/task/2026-08-29_浅色主题大小卡片层级对比.md`。

- ✅ [2026-08-29] 增加服务端 ASR/TTS 独立开关
  - 控制端声纹面板新增“启用服务器语音识别”和“启用服务器语音生成”开关，状态持久化并支持 WebSocket 同步。
  - 关闭后对应服务端设备按钮禁用，当前服务端路由自动切换显示端；无显示端能力时不调用服务端或进行 TTS 回退。
  - 服务端 ASR 关闭时启动阶段不初始化服务端模型，ASR/TTS 请求入口统一增加开关门禁。
  - 验证：新增开关测试 3/3 通过，相关回归测试通过。
...
- ✅ [2026-08-29] 调整控制端媒体播放音量滑条位置
  - 主显示控制区的音量滑条移动到媒体播放进度条下面，保持音量同步、`volume` 控制协议和快捷控制面板不变。
  - 验证：媒体控制布局测试、页面脚本语法检查和 `git diff --check` 通过。
  - 文档：`docs/design/control.md`、`docs/spec/upload.md`、`docs/task/2026-08-29_媒体播放音量滑条布局调整.md`。

- ✅ [2026-08-29] 修复设备列表监听开关并移动语音设备选项
  - 监听复选框补充 `change` 事件，关闭时通过原有 `updateCapabilities` 链路发送 `voiceRecording=false`。
  - 语音识别设备和语音生成设备选项从显示控制区域移动到声纹面板，保留原有配置接口、状态同步和 DOM id。
  - 验证：相关回归测试 26/26 通过，脚本语法检查和 `git diff --check` 通过。
...
- ✅ [2026-08-29] 显示端 ASR 统一公共入口与提供端选择
  - 浏览器和录音 Android 显示端统一采集原始 PCM/WAV 并提交 `/api/asr/recognize`；保留服务端/显示端设备选择。
  - 显示端模式按 `displayClients` 当前连接顺序选择第一个 `voiceRecognition=true` 的提供端，不对 `192.168.1.6` 或其他 IP 特殊优先；被选 APK 才处理 `asrAudio` 并回传 `asrResult`。
  - `voiceRecognition` 表示本地 ASR 提供能力，`voiceSupported` 表示公共 ASR 可用性，录音端不直接调用自身 ASR。
  - 验证：显示端监听契约 27/27、ASR/Android/主题相关回归 45/45，`zh.wav` 公共接口识别成功，显示端内嵌脚本语法检查通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-29_显示端ASR统一服务端路径.md`。

- ✅ [2026-08-29] 修复显示端 ASR 能力状态异步回传不完整
  - 显示端在 ASR 检查、完整能力探测和原生模型 ready/error 后统一补发 `voiceStatus` 与 `capabilities.voiceRecognition`。
  - 修复 `192.168.1.39` 麦克风可用、服务器 ASR 已 ready，但控制端仍显示“监听不可用”的状态竞态。
  - 验证：监听契约 19/19、显示列表/语音会话/监听回传/WebSocket 重连 7/7，显示端内嵌脚本语法检查通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-29_显示端ASR能力状态回传修复.md`。

- ✅ [2026-08-29] 回传显示端所有有效识别文字并只显示最新一条
  - 服务端先广播 `voiceInput`，再过滤声纹未匹配结果；未匹配文字可在控制端观察，但不会触发唤醒、对话或命令处理。
  - 控制端每个显示端继续只保留最新一条识别文本，不累积历史。
  - 验证：监听状态、显示列表、语音会话、监听回传和 WebSocket 重连相关测试 7/7 通过，服务端/控制端脚本语法检查通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-29_显示端识别结果回传最新文本.md`。
...
- ✅ [2026-08-29] 恢复 Android Kotlin 基线代码
  ...
  - `MainActivity` 启动时申请一个共享原生焦点；网页端不重复申请或释放，并通过网页媒体事件和原生通知自动恢复媒体/TTS 播放。
  - 验证：焦点契约 8/8，相关回归退出码 0，APK 构建、覆盖安装和 display 2 启动成功，PID=24605。
  - 真机 `dumpsys audio` 确认启动焦点申请成功；网页媒体播放时仍有 Chromium 内部焦点请求，无法与 APK 原生焦点直接合并。

- ✅ [2026-08-29] 增加网页媒体与 TTS 的失焦自动恢复
  - APK 启动持有一个共享原生焦点，网页不重复申请；视频、普通音频和 TTS 通过媒体事件、watchdog 及原生通知调用网页 `play()` 恢复。
  - 恢复采用单飞定时器和最多 5 次连续重试，避免持续失焦时无限循环。
...
- ✅ [2026-08-29] 接入显示端 CPU 拓扑回报与 TTS 双路生成
  ...
  - 验证：CPU 协议测试 3/3、文本 TTS 测试 28/28、有序调度器/Chat/手动 TTS/Agent 回归 9/9、Android JVM 单测和 Debug APK 构建通过。
  - 真机复测：跳过 `？` 后只生成两段，当前串行总耗时约 2966ms，未再触发服务端回退。
  - 所有 TTS 入口直接跳过仅标点分句，避免单独 `？` 触发无效合成和服务端回退；文本媒体通过可定位通知继续下一句。
  - 文档：docs/design/android-native-tts.md、docs/spec/android-native-tts.md、docs/design/tts.md、docs/spec/tts.md、docs/task/2026-08-29_显示端CPU拓扑回报与TTS并发.md。

- ✅ [2026-08-29] 收敛 TTS 连续仅标点分句
  - 分句结果中如果当前分句和前一个分句都仅包含标点，则删除当前分句，只保留连续标点中的第一个分句。
  - `喵呜……？！ （耳朵瞬间变得通红` 处理为 `喵呜……`、`？`、`（耳朵瞬间变得通红`，避免 `！` 单独进入 TTS 队列。
  - 保持 `你好......世界` 的省略号句间边界行为不变。
  - 文档：`docs/design/tts.md`、`docs/spec/tts.md`、`docs/task/2026-08-29_TTS省略号停顿缩短.md`。

- ✅ [2026-08-29] 缩短 TTS 连续省略号停顿
  - 连续英文句点和中文省略号作为 TTS 句子边界；`你好......世界` 拆为 `你好......`、`世界` 两句，界面原文保持不变。
  - 每个分句的 TTS 输入将省略号边界压缩为单个英文句点，覆盖显示端离线 TTS、服务端 TTS 和 fallback 路由。
  - 实测当前音色：`你好......世界` 分别生成 `你好.`（0.5875 秒）和 `世界`（0.6875 秒）。
  - 验证：TTS 文本处理和分句回归 13/13、语法检查和 `git diff --check` 通过。
  - 文档：`docs/design/tts.md`、`docs/spec/tts.md`、`docs/task/2026-08-29_TTS省略号停顿缩短.md`。

- ✅ [2026-08-29] 修复控制端监听开关并统一 ASR 配置入口
  - 修复树形/列表视图监听开关重复绑定导致的无响应风险，改为单一事件委托；增加发送中、发送失败和服务端回执状态。
  - 删除控制端“显示端 ASR 处理”区域及语言选择，正式显示端固定使用中文 ASR。
  - 降噪开关移动到声纹面板，服务端保存为全局 ASR 降噪配置，普通 ASR、单段声纹和快速多段声纹统一使用。
  - 验证：相关 Node 回归 14/14、JavaScript 语法检查、`git diff --check` 和正式 Display APK 构建通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/design/android-native-asr.md`、`docs/spec/display-voice-conversation.md`、`docs/spec/android-native-asr.md`、`docs/task/2026-08-29_控制端监听开关与全局降噪配置.md`。

- ✅ [2026-08-29] 控制端显示端列表增加监听状态和最近 ASR 回传
  - 每个显示端条目直接显示监听开关、监听中/等待唤醒/已关闭/不可用状态和最近一次实时/最终识别文本。
  - 监听开关复用 `voiceRecording` 能力，不新增控制协议；最近文本按显示端只保留一条。
  - 验证：显示列表回归 2/2、相关网页/语音监听/录音回归 9/9、JS 语法检查和正式 APK 构建通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-29_显示端监听状态与最近识别文本.md`。

- ✅ [2026-08-29] 删除 ASR“过滤其他文字”开关和 `zh-en-filter` 模式
  - 测试 APK、正式 Display APK、控制端配置和服务端 ASR 链路不再保存、广播或执行文字脚本过滤。
  - 保留 `auto`、`zh`、`en` 语言选择、降噪、快速多段和流式 ASR；识别结果只做首尾空白清理。
  - 验证：相关 Node 契约 3/3、两个 Android JVM 单测和正式 Display APK Debug 构建通过。
  - 文档：`docs/design/android-native-asr.md`、`docs/spec/android-native-asr.md`、`docs/task/2026-08-29_删除ASR其他文字过滤.md`。

- ✅ [2026-08-29] 服务器 TUI 默认关闭
  - `npm start` 默认不启用 TUI，只有显式传入 `--tui` 才启用；`--no-tui` 继续作为强制关闭参数。
  - 验证：相关启动器、重启脚本和 WebSocket 重连测试 12/12 通过，服务端脚本语法检查通过。
  - 文档：`docs/design/server-restart-script.md`、`docs/spec/server-restart-script.md`、`docs/task/2026-08-29_服务器TUI默认关闭.md`。

- ✅ [2026-08-29] 新增服务器双进程启动器与控制台重启
  - 新增前台 `server-launcher.js`，服务器子进程继承控制台标准流和 IPC；主动重启由启动器重新拉起，普通停止不自动重启。
  - `/api/restart` 不再 detached 启动新进程，改为通知启动器，并在退出前关闭 WebSocket、TUI 和 Chat 运行时；服务器直接运行时保留仅退出当前进程的兼容行为。
  - 异常退出最多延迟重启 5 次，服务器发送 `serverReady` 后清零计数；验证相关测试 11/11 通过。
...
- ✅ [2026-08-29] 测试 ASR APK 增加保存当前 WAV
  - 内置测试网页和 Android 原生界面均增加“保存当前 WAV”按钮；没有当前音频时按钮不可用。
  - Android 10+ 写入系统 Download，旧系统写入 APK 专属外部音频目录；保存内容统一为 16kHz mono 16-bit PCM WAV。
  - 验证：网页保存契约 3/3、Android JVM 单元测试、debug APK 构建通过。
  - 文档：`3rd/tts-server/docs/design/android-asr-apk.md`、`3rd/tts-server/docs/spec/android-asr-apk.md`、`3rd/tts-server/docs/task/2026-08-28_测试ASR保存当前WAV.md`。

- ✅ [2026-08-29] 测试 ASR 普通识别固定使用中文
  - 内置网页删除语言选择，普通 ASR 和声纹测试固定传 `language=zh`；Android 原生识别固定使用 `AsrLanguageMode.ZH`。
  - 流式 ASR 继续使用固定中英双语 Zipformer，HTTP 接口仍保留显式语言参数兼容性。
  - 验证：网页语言契约 4/4、Android JVM 单元测试、debug APK 构建和真机安装通过。
  - 文档：`3rd/tts-server/docs/design/android-asr-apk.md`、`3rd/tts-server/docs/spec/android-asr-apk.md`、`3rd/tts-server/docs/task/2026-08-29_测试ASR普通识别固定中文.md`。
...
- ✅ [2026-08-28] 显示端媒体底色恢复固定黑色
  - `html/body`、`#mediaContainer` 和媒体睡眠遮罩不再使用主题 `bg-primary`，媒体未覆盖区域始终保持黑色。
  - 显示端连接状态、文本媒体、天气/语音弹窗和任务提示继续使用共享主题变量；图片、视频和 iframe 内容不变。
...
- ✅ [2026-08-28] 统一所有 ASR/声纹录音为原始 PCM/WAV
  - 控制端语音输入、声纹注册、正式显示端和独立 `ttslive` 网页均改用 WebAudio 原始 Float32 采集，统一封装为 16 kHz mono 16-bit WAV。
  - Node 子显示端保留现有 WAV 录音器，Go/C# 子显示端保留 `audio.wav` 上传契约；网页端不再使用 MediaRecorder/WebM。
  - 验证：网页录音契约测试 8/8 通过，控制端、显示端、声纹注册和 ttslive 入口均通过 WAV 静态检查。
  - 文档：`docs/design/android-native-asr.md`、`docs/spec/android-native-asr.md`、`docs/task/2026-08-28_APK内存不足ASR重连与正式识别准确度修复.md`。

- ✅ [2026-08-28] 修复正式 Display APK ASR 内存不足重连并对齐测试 APK 音频链路
  - 正常情况下保留按 CPU 核心数创建 recognizer；内存预算不足时回退为单实例，recognizer 构造抛 `OutOfMemoryError` 时释放已创建 slot 后再次尝试单实例。
  - 单实例仍无法创建时报告内存终态，显示端停止录音、撤销 `voiceRecognition` 能力并停止自动模型重试，不关闭 WebSocket。
  - APK 本地 ASR 使用 WebAudio 原始 Float32 采集并封装 16 kHz mono WAV，关闭浏览器层回声消除/噪声抑制请求；普通浏览器同样使用 WAV。
  - 验证：Android JVM 全量单测、网页音频契约测试、`npm run build:apk`、SM-N9500 安装启动通过；启动后无 FATAL/OOM/ASR 重连日志。
  - 文档：`docs/design/android-native-asr.md`、`docs/spec/android-native-asr.md`、`docs/task/2026-08-28_APK内存不足ASR重连与正式识别准确度修复.md`。

- ✅ [2026-08-28] 统一基础主题色并让显示端信息、设备树控件继承主题
  - 显示端信息弹窗、浏览器详情、设备树、树状节点和树状输入控件改用基础主题变量或父级继承色，修复浅色主题下白字/灰字对比度异常。
  - 保留在线、支持/不支持、选中和危险操作的语义颜色；输入框、下拉框和滑条使用主题控件背景、边框和焦点色。
  - 验证：主题回归 `tests/ui-theme.test.js` 19/19 通过。
  - 文档：`docs/design/control-ui-theme.md`、`docs/spec/ui-theme.md`、`docs/task/2026-08-28_基础主题色与控件继承.md`。

- ✅ [2026-08-28] 修复显示端透明背景和服务启动异常
  - 修复 `config-app-service.js` 导出未定义 `normalizeControlTheme` 导致服务端启动崩溃的问题。
  - 连接状态、文本媒体、语音提示、任务状态和天气/语音弹窗改用不透明的 `--bg-secondary` 主题表面，避免深色主题半透明变量造成透底。
  - 图片、视频和 iframe 网页内容边界保持不变。
  - 验证：配置服务、主题同步、显示端旋转/播放恢复和文本媒体相关回归 45/45 通过；`node --check` 与 `git diff --check` 通过。
  - 文档：`docs/design/control-ui-theme.md`、`docs/spec/ui-theme.md`、`docs/task/2026-08-28_显示端透明背景与服务启动修复.md`。

- ✅ [2026-08-28] 显示端文件变化检测间隔调整为 30 秒
  - `/api/display-version` 轮询从 8 秒调整为 30 秒，成功检查和失败重试保持一致，减少版本检测请求频率。
  - 检测到 `public` 目录文件版本变化仍立即刷新显示端页面；控制端不新增独立文件变化轮询。
  - 测试：显示端版本检测成功间隔、失败重试间隔、版本变化刷新和控制端无轮询检查通过。
  - 文档：`docs/design/display.md`、`docs/spec/api.md`、`docs/task/2026-08-28_显示端文件变化轮询改为30秒.md`。

- ✅ [2026-08-28] 增加显示端语音唤醒、监听开关和 TTS 后超时控制
  - 控制端复用每个显示端的“语音监听（录音能力）”开关；关闭后立即停止麦克风、本地 ASR 和自动恢复，未完成的语音结果不再触发命令。
  - 正式网页显示端增加等待唤醒、群聊激活、私聊激活和结束对话状态；支持“你好小爱”“小爱你好”“{助手}开始对话”“结束对话”等流程，3 分钟无有效对话后重新等待唤醒。
  - TTS 队列全部实际播放结束后由显示端回报，服务端和显示端从最后一段 TTS 结束时重新计时；关闭声纹只取消 speaker 匹配，不影响 ASR、唤醒和对话。
  - 旧 Go/C#/Node 子显示端保留原有语音命令兼容流程。
  - 验证：状态机 4/4、监听契约 8/8、显示端重连 2/2、TTS 路由 3/3、原生桥 4/4、ASR 配置 3/3、播放恢复 5/5 通过；服务端/模块/显示端脚本语法检查通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/spec/websocket.md`、`docs/design/display-capability.md`、`docs/task/2026-08-28_显示端语音唤醒监听控制.md`。

- ✅ [2026-08-28] 显示端跟随服务端主题颜色变化
  - 显示端加载共享 `theme.css` 与 `ui-theme.js`，首次连接、重连和控制端切换均使用服务端全局主题；主题更新通过 `controlThemeChanged` WebSocket 消息即时生效。
  - 显示端背景、文本媒体、设备/连接状态文字、语音提示、任务状态、监控条和天气/语音等临时弹窗改用主题变量，覆盖当前 15 种主题。
  - 图片、视频、iframe 网页及其内部内容不套用主题滤镜或样式，保持媒体原样显示。
  - 验证：主题、显示端同步、旋转弹窗、文本媒体和播放恢复相关回归 44/44 通过；`node --check` 与 `git diff --check` 通过。
  - 文档：`docs/design/control-ui-theme.md`、`docs/spec/ui-theme.md`、`docs/task/2026-08-28_显示端主题同步.md`。

- ✅ [2026-08-28] 主题设置保存到服务端并修复天气响应弹窗旋转适配
  - 服务端新增 `ui.controlTheme` 配置和 `/api/config/controlTheme` 读写接口，限制为控制端当前 15 种主题；控制端启动优先同步服务端主题，保留 `localStorage` 离线回退。
  - 明确“今日天气”显示控件为显示端动态 `.voice-response-popup`，并将语音响应、确认、搜索结果、播放选择和提醒弹窗统一纳入旋转布局。
  - 0/90/180/270 度下弹窗整体随角度旋转，90/270 度使用交换后的逻辑宽高限制长文本区域。
  - 测试：主题、服务端白名单、显示端旋转及文本媒体相关回归共 41 项通过；脚本语法检查和 `git diff --check` 通过。
  - 文档：`docs/design/control-ui-theme.md`、`docs/spec/ui-theme.md`、`docs/task/2026-08-28_主题服务端持久化与天气弹窗旋转适配.md`。

- ✅ [2026-08-28] 补齐控制端其他状态按钮的浅色选中态
  - 语音识别/生成设备按钮改为同步 `active` class；自动播报、文本播放和批量播放同步 active/playing/paused 状态。
  - 设备选择模式、日志筛选/自动滚动和任务选项增加浅色主题中性未选中态与强调色选中态。
  - 测试：`tests/ui-theme.test.js` 主题回归 15/15 通过。
  - 文档：`docs/design/control-ui-theme.md`、`docs/spec/ui-theme.md`、`docs/task/2026-08-28_控制端其他状态按钮选中态修复.md`。

- ✅ [2026-08-28] 修复显示控制面板 toggle/状态按钮浅色选中态
  - 画面填充、裁剪旋转、中心缩放及浮动控制面板的填充按钮，统一区分未选中和选中状态。
  - 主面板播放/暂停与浮动面板播放状态使用主题成功色/危险色，避免被浅色通用按钮样式覆盖。
  - 测试：`tests/ui-theme.test.js` 主题回归 14/14 通过。
  - 文档：`docs/design/control-ui-theme.md`、`docs/spec/ui-theme.md`、`docs/task/2026-08-28_显示控制toggle选中态修复.md`。

- ✅ [2026-08-28] 修复浅色主题 toggle 与 LLM/系统路由按钮无法区分选中状态
  - 自定义 toggle 的关闭态使用浅灰轨道/边框和可见滑块，开启态使用主题强调色轨道与白色滑块；原生复选框和单选框同步使用主题强调色。
  - 设置页天气、搜索的 LLM/系统按钮增加 `active` 状态；浅色主题下未选中项使用浅色背景，选中项使用主题渐变和强调色外框。
  - 测试：`tests/ui-theme.test.js` 主题回归 13/13 通过。
  - 文档：`docs/design/control-ui-theme.md`、`docs/spec/ui-theme.md`、`docs/task/2026-08-28_浅色主题toggle选中态修复.md`。

- ✅ [2026-08-28] 修复控制端聊天群聊页签无效并增加模式退出按钮
  - 修复群聊页签只有 `data-chat-tab` 标记却没有点击事件，现可从私聊或工作组模式直接返回群聊。
  - 私聊模式显示“退出私聊”，工作 AI 角色模式显示“退出工作组”，均复用 `Chat.setMode('group', null)` 清理上下文。
  - 测试：聊天/Agent 控制端回归 6/6、脚本语法检查和 `git diff --check` 通过。
  - 文档：`docs/design/chat-system.md`、`docs/spec/chat-system.md`、`docs/task/2026-08-28_控制端聊天模式页签切换与退出按钮.md`。

- ✅ [2026-08-28] 将测试 APK 的 ASR 增强能力接入正式 Display APK
  - 正式 APK 增加普通非流式 ASR 的自动/中文/英文/中英双语语言选择；中英双语结果过滤日文假名、韩文等其他脚本文字。
  - 集成 Sherpa GTCRN 降噪开关；一次请求只降噪一次，普通 ASR、声纹和快速多段共用降噪后的 PCM。
  - 集成 Sherpa 快速多段声纹流程，按 cluster 选择最长代表片段提取 embedding，人数支持 AUTO 或 1～5。
  - 网页、服务端配置、显示端 WebSocket 和原生桥完成扩展；保留旧异步桥和旧配置兼容，不接入流式 ASR。
  - 验证：正式 APK Android JVM 全量单测通过，新增 Node 契约测试 9/9，通过 `node --check` 和 `npm run build:apk`；APK 已确认包含 GTCRN 资源。
  - 文档：`docs/design/android-native-asr.md`、`docs/spec/android-native-asr.md`、`docs/task/2026-08-28_正式APK接入测试APK的ASR增强功能.md`。

- ✅ [2026-08-28] 测试 APK 增加当前音频播放和中英混合识别后过滤
  - 原生 APK 增加当前选择/录制音频播放和停止按钮；测试网页增加 WAV 播放器、播放/停止按钮，并在替换音频时释放 object URL。
  - 离线 ASR 支持 `auto`、`zh`、`en` 和 `zh-en-filter`；中英混合模式使用 SenseVoice auto 后过滤日文假名、韩文等其他脚本，声纹分段 ASR 同步应用过滤。
  - `/api/asr`、`/api/voiceprint/test` 增加 `language` 查询参数；语言切换只保留一个 SenseVoice recognizer，避免多份模型常驻。
  - 验证：独立 APK 29 项 JVM 单测、网页脚本语法检查、Debug APK 构建和 `192.168.1.6:5555` 真机安装通过；原生按钮手工点击因设备锁屏未完成。
  - 文档：`docs/design/android-voiceprint-test-apk.md`、`docs/spec/android-voiceprint-test-apk.md`、`docs/task/2026-08-28_测试APK当前音频播放与中英混合过滤.md`。

- ✅ [2026-08-28] 扩展控制端多主题配色
  - 新增薄荷青、海洋蓝、森林绿、极简灰、藻盐、少女粉、玫瑰金和新年红八套浅色主题，连同已有主题共支持 15 种配色。
  - 藻盐主题采用盐白背景与低饱和藻绿强调色，推荐主背景 `#E8F6F0`、强调色 `#159A86`。
  - 少女粉主题使用更明亮的粉红层级，与现有淡粉主题区分。
  - 玫瑰金使用香槟白与粉铜色，新年红使用暖白、节庆红和深酒红，避免大面积高饱和色造成刺眼效果。
  - 新主题复用卡片、弹窗、设备名、聊天消息和日志的浅色对比度规则，按钮渐变改用主题强调色变量。
  - 测试：`tests/ui-theme.test.js` 主题回归 11/11 通过，覆盖 15 种主题。
...
- ✅ [2026-08-27] 独立声纹测试 APK 增加 Sherpa GTCRN 降噪开关
  - 网页增加“ASR/声纹降噪”开关，覆盖声纹注册、Sherpa 单段、普通多段和快速多段；流式 ASR 保持不变。
  - 开启时完整音频只执行一次 GTCRN 降噪，降噪音频统一用于 diarization、embedding、声纹匹配和 ASR；结果增加 `denoise`、`denoiseMs`。
  - 真机 SM-N9500 验证 `zh.wav` 四个降噪请求均成功，注册/单段/普通多段/快速多段降噪耗时分别为 `1556/1536/1542/1615ms`，均匹配 `ZH`。
  - 改动：`3rd/tts-server/android-asr/`、`res/models/speech-enhancement/gtcrn_simple.onnx`；文档：`docs/design/android-voiceprint-test-apk.md`、`docs/spec/android-voiceprint-test-apk.md`、`docs/task/2026-08-27_独立APK声纹测试降噪开关.md`。
...
- ✅ [2026-08-27] 补充 Sherpa 混合语音处理验证
  - `zh-en-mix.wav` 多段返回重叠的 EN/ZH 时间段，但不执行 source separation；两个分段 ASR 均被英文主导。
  - 快速 2 人模式 `11920ms`，普通多段 `11955ms`，混合输入只有两个分段时优化收益很小。
...
- ✅ [2026-08-26] 修正 90°/270° 旋转文本四角位置偏移
  ...
  - 测试：四角锚点回归 7/7、相关静态与逻辑回归 41/41、显示端集成回归 9/9 通过。
  - 文档：`docs/design/display.md`、`docs/spec/display-ui-rotation.md`、`docs/task/2026-08-26_显示端旋转角落位置校正.md`。

- ✅ [2026-08-26] 修正显示端旋转布局宽高交换与中文文本越界
  - 90°/270°计算旋转后的逻辑画布，交换视口宽高并按交换后的尺寸重新设置固定文本最大宽高和边距。
  - 连接状态、时间、媒体名、语音状态和播报文本统一按旋转角度整体变换，并基于实际包围盒校正固定定位；内容变化和 resize 会触发布局重算。
  - 固定文本过渡仅保留颜色、透明度等视觉属性，避免尺寸和位置动画产生越界中间态。
  - 测试：旋转布局回归 6/6、相关静态与逻辑回归 40/40、显示端集成回归 9/9 通过。
  - 文档：`docs/design/display.md`、`docs/spec/display-ui-rotation.md`、`docs/task/2026-08-26_显示端旋转布局宽高交换修正.md`。

- ✅ [2026-08-26] 修复文本媒体 TTS 固定旧设备且仅从选中设备选择
  - `text-media-tts-service.js` 每个实际句子重新检查全部在线且启用 `voicePlayback` 的显示端；源显示端不可播时可选择未选中的语音设备，连续句子间按最新能力切换目标。
  - 预取句沿用当前句实际目标；目标失效时取消预取，不把缓存改投其他设备，实际下一句重新选择目标。远程完成回执按句子实际目标校验，动态换目标后取消/断连仍能清理对应远程设备。
  - 验证：文本媒体 TTS、分页、服务器协议和集成回归 86/86 通过，`node --check` 与 `git diff --check` 通过。
...
- ✅ [2026-08-22] 修复控制端 Agent 回复不会自动语言播报
  ...
  - 验证：Agent TTS 测试 4 项、AI 角色服务/WS/角色存储测试通过；Codex/pipe-keeper 进程测试受当前沙箱限制未通过。
  - 文档：docs/design/ai-roles.md、docs/spec/ai-roles.md、docs/task/2026-08-22_控制端Agent回复自动播报.md

- ✅ [2026-08-22] 修复 Codex 响应超时导致 Agent 离线
  - 默认超时调整为 10 分钟，并改为无活动超时。
  - 持续收到流式增量时自动续期，正常收到完成事件后保持在线。
...
- ✅ [2026-08-20] 修复 APK 显示端被系统媒体播放器抢占后视频/音频暂停且播放状态不同步
  ...
  - 系统媒体抢占期间保持服务器 `isPlaying=true`；控制端手动暂停、睡眠和批量暂停设置明确拦截，不会被自动拉起。
  - 验证：显示端媒体焦点静态测试 3 项通过；APK Android JVM 测试 24 项通过；显示端内嵌脚本语法通过。
  - 说明：全量 Node 测试中 `display-native-bridge.test.js` 的既有触摸注入断言失败，现有实现仅将键盘/文本走原生桥，未由本次媒体焦点改动引入。

- ✅ [2026-08-20] 调整控制端音频选择按钮背景样式，使用独立蓝紫渐变区分音频入口
...
以下 173 条已完成任务从 `docs/todo.md` 迁移至此，保留原任务描述。


- ✅已完成 [2026-08-31][2026-08-31] 任务列表轮询日志降噪
  - 静默正常 `task:list` 的 WebSocket 收发日志，保留任务列表处理异常日志。
  - 测试：任务列表/Widget 日志降噪专项 3/3 通过。
  - 关联任务：`docs/task/2026-08-31_任务列表轮询日志降噪.md`。

- ✅已完成 [2026-08-30][2026-08-30] 任务 widgetRefresh 高频日志降噪
  - 静默运行中 widgetRefresh 的 WebSocket 收发日志和 task:widget_update 广播日志，保留人工 widget 操作、错误和任务状态日志。
  - 测试：widget 日志降噪专项 2/2 通过。
  - 关联任务：`docs/task/2026-08-30_任务widget刷新日志降噪.md`。

- ✅已完成 [2026-08-30][2026-08-30] 修复 Chat2API 账户管理页面主题适配
  - 移除账户管理弹窗和动态表单中的固定深色内联样式，新增独立 `chat2api.css` 并复用控制端主题变量。
  - 支持弹窗、分区、输入框、列表、状态文字、按钮和窄屏布局跟随主题切换。
  - 测试：Chat2API 控制端 UI 契约和主题样式检查通过。
  - 关联任务：`docs/task/2026-08-30_Chat2API核心内置任务.md`。

- ✅已完成 [2026-08-30][2026-08-30] 增加 Chat2API 模型映射手动编辑
  - 账户管理页面支持新增、编辑、删除模型映射，可设置请求模型、实际模型、优先 Provider 和优先账号。
  - 编辑请求模型时处理旧映射键，避免修改后保留重复旧记录。
  - 测试：控制端 Chat2API UI 契约和完整 Chat2API 回归通过。
  - 关联任务：`docs/task/2026-08-30_Chat2API核心内置任务.md`。

- ✅已完成 [2026-08-30][2026-08-30] 修复原 Chat2API 用户模型映射未迁移
  - 读取 `userModelOverrides.*.addedModels`，将显示名称、实际模型 ID 和 Provider 转换为 AASC 模型映射。
  - 原配置中的 `Qwen3.6-Flash → Qwen3.7` 现在会进入导入预览并在确认后写入。
  - 测试：数据迁移回归验证 6/6 通过。
  - 关联任务：`docs/task/2026-08-30_Chat2API核心内置任务.md`。

- ✅已完成 [2026-08-30][2026-08-30] 修复 Chat2API 用户模型映射无可用账号
  - 模型解析同时兼容 `preferredProviderId` 和导入生成的 `providerId`。
  - 指定 Provider 的用户自定义模型不再要求出现在 Provider 内置模型清单中，仍会校验 Provider、账号启用状态和账号健康状态。
  - 测试：模型映射、负载均衡和 Chat2API 完整回归通过。
  - 关联任务：`docs/task/2026-08-30_Chat2API核心内置任务.md`。

- ✅已完成 [2026-08-30][2026-08-30] 修复 Qwen 模型返回空内容
  - 按原版 Chat2API 发送 `/api/v2/chat` 专用请求体、查询参数和 `tongyi_sso_ticket` Cookie，不再发送会触发签名错误的 Bearer ticket。
  - Qwen 响应按 `content-encoding` 解压 gzip/deflate/br，再统一按 SSE 读取；非流式请求聚合 `data.messages` 后返回 OpenAI `chat.completion`。
  - 流式响应按累计内容只输出新增文本，并过滤 `deep_think` 等内部标记。
  - 测试：Qwen 适配器 6/6 通过；真实 Qwen3.7 上游返回 `测试成功`。
  - 关联任务：`docs/task/2026-08-30_Chat2API核心内置任务.md`。

- ✅已完成 [2026-08-30][2026-08-30] 接入 Chat2API 其他 Provider 原版主协议
  - DeepSeek、GLM、Kimi、MiMo、MiniMax、Perplexity、Qwen AI、Z.ai 使用各自的请求体、认证头、会话/设备预处理和响应事件解析。
  - 增加 Kimi gRPC-Web 帧、MiMo/Perplexity/Qwen AI/Z.ai phase SSE、MiniMax详情轮询及 DeepSeek PoW WASM 内置资源。
  - 测试：8 个 Provider 专用模拟协议回归和 Chat2API 全量回归通过；真实 Qwen 已验证，其他 Provider 需对应账号做真实回归。
  - 关联任务：`docs/task/2026-08-30_Chat2API核心内置任务.md`。

- ✅已完成 [2026-08-30][2026-08-30] 增加 Chat2API 原始请求响应调试日志
  - 控制端可开启原始请求/响应日志并设置单次最大字节数，默认关闭；日志覆盖 Provider 会话预处理、最终请求和详情轮询。
  - URL、请求头、请求体和响应内容按敏感字段脱敏；流式响应原样透传，超过预算只保留一条截断标记，不影响请求。
  - 测试：原始流量追踪器、Provider 集成、管理配置和控制端契约测试通过。
  - 关联任务：`docs/task/2026-08-30_Chat2API原始请求响应调试日志.md`。

- ✅已完成 [2026-08-31][2026-08-31] 修复 Chat2API 模型映射未传递到 Provider
  - 负载均衡候选阶段复用统一模型映射器，按原版优先级解析 Provider 内置映射、全局映射和原始模型。
  - `Qwen3.6-Flash → Qwen3.7` 现在会进入候选的 `actualModel`，原始请求日志中的请求体将显示实际发送模型。
  - 测试：负载均衡专项和 Chat2API 全量回归通过。
  - 关联任务：`docs/task/2026-08-31_Chat2API模型映射优先级对齐原版.md`。

- ✅已完成 [2026-08-30][2026-08-30] 系统搜索内置任务取十条并展示五条
  - 搜索拆为支持手动调用的 `search.web` 单次内置任务；任务只用 Axios 取前十条真实结果并按摘要日期排序，返回前五条。
  - 语音调用复用任务引擎，统一使用 `data.result` 数组；搜索频道/历史/弹窗展示五条，TTS 播报前三条；任务本身不产生外部副作用。
  - 关联任务：`docs/task/2026-08-30_系统搜索取三条并按时间选择.md`。

- ✅已完成 [2026-08-30][2026-08-30] 修复等待唤醒确认词误匹配
  - 确认/拒绝/取消免唤醒指令改为完整短指令匹配；“拒绝奖励出街注意”等普通语音不会再通过 waitingWake 门控。
  - 测试：内置指令和显示端语音会话回归 29/29 通过。
  - 关联任务：`docs/task/2026-08-30_等待唤醒确认词误匹配.md`。

- ✅已完成 [2026-08-30][2026-08-30] 搜索频道与独立 Pi 进程
  - 控制端新增独立搜索频道，搜索状态和历史不再嵌入群聊；修复历史消息收到后未刷新问题。
  - LLM/Pi 搜索只发送当前请求，每次使用一次性 Pi 进程，完成、失败或超时后自动回收。
  - 测试：Pi Runtime、LLM Agent、搜索频道、语音和 Markdown 控制端回归 47/47 通过，脚本语法检查通过。
...
- ✅已完成 [2026-08-30][2026-08-30] 增加全局语音对话确认三档模式
  ...
  - 确认文本使用天气同款弹窗显示，确认倒计时追加在唤醒倒计时后并带背景。
  - 控制端入口位于“声纹管理 → 声纹识别设置”卡片内，作为全局语音设置的一部分。
  - 自动确认等待所有实际 TTS 播放目标完成后再开始 7 秒取消窗口；声纹页面主要设置统一使用大卡片。
  - 确认倒计时使用深浅主题均可读的高对比背景和文字颜色。
  - TTS 失败时保留弹窗和待确认状态，不会立即污染聊天或 Pi 历史；自动模式随后进入 7 秒取消窗口，旧的 5 秒自动确认提示已移除。
  - 测试：确认状态机、语音路由、TTS、Pi 和显示端相关回归全部通过。
  - 关联任务：`docs/task/2026-08-30_语音对话确认模式.md`。

- ✅已完成 [2026-08-30][2026-08-30] 显示端第二行显示唤醒退出倒计时
  - 第一行保持状态+柱状图，第二行只显示 `MM:SS`，使用服务端到期时间同步。
  - 会话结束或监听关闭时清理倒计时定时器；旧服务端缺少到期时间时兼容隐藏。
  - 测试：显示端倒计时、旋转布局和语音状态回归 12/12 通过。
  - 关联任务：`docs/task/2026-08-30_显示端唤醒退出倒计时.md`。

- ✅已完成 [2026-08-30][2026-08-30] 其他设备播报文件名时暂停非声纹监听
  - 文件名自动播报改走服务器统一 TTS，复用播放 ID 和跨显示端播放状态广播。
  - 其他显示端在非声纹模式下收到播报状态会暂停监听，相关播报完成后恢复；本地文件名播报不再绕过服务器。
  - 测试：文件名 TTS、跨设备 TTS、显示端 UI、语音会话和文本媒体回归 13/13 通过。
  - 关联任务：`docs/task/2026-08-30_其他设备文件名播报暂停监听.md`。

- ✅已完成 [2026-08-30][2026-08-30] 修复妲己私聊会话列表只显示默认会话
  - 服务端合并客户端会话快照，避免控制端异步加载期间覆盖已有会话；列出会话时从仍存在的私聊历史恢复缺失条目。
  - 前端普通状态保存不再提交 sessions 元数据；现有妲己历史会话 `mp454fe0e8dz` 可恢复，原名称缺失时显示 sessionId。
  - 测试：私聊会话恢复专项 3/3、LLM 回归 4/4 通过，语法检查和 `git diff --check` 通过。
  - 关联任务：`docs/task/2026-08-30_妲己私聊会话恢复.md`。

- ✅已完成 [2026-08-30][2026-08-30] Pi 语音会话按群聊/私聊角色隔离并修复唤醒群聊路由
  - 群聊共用一个 Pi 进程，私聊按角色/会话独立；切换模式、角色或会话重置旧进程但保留应用层历史。
  - 修复音频流 ASR 在群聊唤醒状态下仍受指令模式拦截。
  - 修复会话状态对象取值错误，并增加 voiceCommand 门控诊断日志。
  - 测试：语音路由、私聊元数据、Pi 会话切换回归 15/15 通过。
  - 关联任务：`docs/task/2026-08-30_Pi语音会话隔离与唤醒群聊路由.md`。

- ✅已完成 [2026-08-30][2026-08-30] 天气和语音详情弹窗支持 Markdown
  - 显示端复用安全 `ChatMarkdown` 渲染器，支持标题、列表、表格、引用和代码块；原始 HTML 和危险链接不会执行。
  - TTS 继续使用纯文本，弹窗显示时长、滚动和旋转布局保持不变。
  - 测试：Markdown、显示端 UI、语音会话和普通聊天回归 7/7 通过。
  - 关联任务：`docs/task/2026-08-30_天气弹窗支持Markdown.md`。

- ✅已完成 [2026-08-30][2026-08-30] 修复唤醒后群聊不入聊天与 Chat2API 查找工具失败
  - 显示端 activeGroup/activePrivate 普通语音不再被全局指令模式拦截，继续进入正常聊天路径；内置命令仍优先，顶部状态显示群聊或私聊目标。
  - Chat2API `find` 统一映射到不依赖 `fd` 的 `aasc_find`，使用 Node 文件系统只读遍历；兼容旧式参数和命名参数 CDATA 格式。
  - 测试：相关全量回归 70/70 通过，Node 语法检查和 `git diff --check` 通过。
  - 关联任务：`docs/task/2026-08-30_唤醒后群聊与Chat2API查找工具修复.md`。

- ✅已完成 [2026-08-30][2026-08-30] Pi Agent 无回复自动恢复与排队保护
  - Pi 请求排队默认 30 秒超时；空回复、RPC 错误、错误型 `agent_end` 和进程异常会回收旧 session，并最多自动用新 session 重试一次。
  - 进程提前退出、排队任务切换会共享重试上限，避免 Pi Agent 无限重建或后续请求长期卡住。
  - 测试：Pi Runtime 16/16、聊天与 Agent 综合回归 57/57、显示端语音子用例 8/8 通过，语法检查和 `git diff --check` 通过。
  - 关联任务：`docs/task/2026-08-30_PiAgent无回复自动恢复.md`。

- ✅已完成 [2026-08-30][2026-08-30] 修复显示端纯唤醒误进入聊天并屏蔽 Pi 详细日志
  - 等待唤醒时纯“你好，小爱。”只进入群聊，不再转发为普通聊天；服务器默认不输出 Pi Agent 生命周期明细。
  - 测试：显示端语音、Pi Runtime 和聊天回归 54/54 通过，脚本语法检查和 `git diff --check` 通过。
  - 关联任务：`docs/task/2026-08-30_显示端纯唤醒与Pi日志屏蔽.md`。

- ✅已完成 [2026-08-30][2026-08-30] 统一 Pi Agent 历史角色标记
  - Pi Agent 续聊消息统一补充 `User:`；Pi 初始化历史使用 `System:`、`User:`、`Assistant:`，普通 LLM 的旧 raw 格式保持不变。
  - 测试：Pi Runtime、LLM Agent 角色标记和聊天回归 21/21 通过，脚本语法检查和 `git diff --check` 通过。
  - 关联任务：`docs/task/2026-08-30_Pi历史角色标记统一.md`。

- ✅已完成 [2026-08-30][2026-08-30] Pi Agent 会话历史递归与聊天单轮删除
  - Pi Agent 仅首次初始化必要历史，后续只发送当前消息；群聊和不同私聊 session 使用隔离的 Pi 内存会话。
  - 控制端支持删除任意中间一轮用户/助手对话，删除后重置对应 Pi 会话。
  - 测试：Pi、LLM、聊天删除和控制端契约回归 51/51 通过，脚本语法检查和 `git diff --check` 通过。
  - 关联任务：`docs/task/2026-08-30_PiAgent会话去重与聊天单轮删除.md`。

- ✅已完成 [2026-08-30][2026-08-30] Pi Agent 偶发无回复与控制端聊天回包诊断
  - Pi Runtime 记录 requestId 入队、启动、RPC 事件、完成、失败和超时；错误型或空 `agent_end` 统一回传失败，不再产生空成功回复。
  - 控制端聊天入站内容和 `chatResponse` 记录 requestId、状态和长度，可区分 Pi 未回复、服务器失败和前端未匹配。
  - 测试：Pi Runtime、Agent 配置、Chat2API、控制端回包相关回归 41/41 通过，脚本语法检查和 `git diff --check` 通过。
  - 关联任务：`docs/task/2026-08-30_PiAgent无回复诊断与控制端聊天日志.md`。

- ✅已完成 [2026-08-30][2026-08-30] 添加 Chat2API 到 Pi 的工具转换
  - Chat2API 标签式工具调用已在 Pi Provider 边界转换为原生 `toolCall`，Pi 可继续执行只读工具循环，协议标签不再进入聊天文本和 TTS。
  - 测试：新增转换/扩展测试 11/11，现有 Agent/聊天回归总计 31/31；真实 Pi 0.84.2 假 SSE 成功执行 `read`，语法检查和 `git diff --check` 通过。
  - 关联文档：`docs/design/llm-agent-mode.md`、`docs/spec/llm-agent-mode.md`、`docs/task/2026-08-30_添加Chat2API到Pi工具转换.md`。
...
- ✅已完成 [2026-08-30][2026-08-30] 系统帮助分句 TTS 批次完成状态
  ...
  - 测试：系统帮助分句、批次完成状态、显示端语音会话、普通聊天和 TTS 路由回归 17/17 通过，服务端脚本语法检查通过。
  - 关联文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-29_系统指令动态播报全部命令.md`。

- ✅已完成 [2026-08-30][2026-08-30] 系统帮助 TTS 按句分段生成播放
  - 系统帮助文本复用句子切分和串行 TTS 调度器，逐句生成并按原始顺序播放，避免单次请求携带超长帮助文本。
  - 控制端定向帮助和显示端通用帮助均保持原有播放目标；控制端帮助弹窗仍显示完整文本。
  - 测试：内置语音指令、显示端语音会话、普通聊天和 TTS 路由回归 16/16 通过，服务端脚本语法检查通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-29_系统指令动态播报全部命令.md`。
...
- ✅已完成 [2026-08-30][2026-08-30] 修复多显示端与多来源 render-display 链接冲突
  ...
  - 测试：任务链接、面板和任务恢复回归 15/15 通过。
  - 文档：`docs/design/monitor-system.md`、`docs/spec/monitor-system.md`、`docs/task/2026-08-30_任务链接面板统一管理.md`。

- ✅已完成 [2026-08-30][2026-08-30] 选择目标后即时刷新任务链接列表
  - 选择目标后立即从添加列表移动到已链接列表，按钮切换为“解除”，弹窗保持打开。
  - 测试：任务链接、链接面板和任务恢复回归 12/12 通过。
  - 文档：`docs/design/monitor-system.md`、`docs/spec/monitor-system.md`、`docs/task/2026-08-30_任务链接面板统一管理.md`。

- ✅已完成 [2026-08-30][2026-08-30] 缩小显示端语音柱状图并稳定识别文本定位
  - 柱状图宽度调整为 140px，监听状态和柱状图继续不换行整体居中；识别文本保持独立定位。
  - 测试：显示端旋转、语音 UI 和监听回归通过，页面内联脚本编译和 `git diff --check` 通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-30_显示端语音状态旋转与监视图动画.md`。

- ✅已完成 [2026-08-30][2026-08-30] 修复旋转后语音文字重复排版导致的位置跳动
  - 语音状态行和识别文字按当前实际尺寸直接计算 90°/270° 边缘中心锚点，避免历史补偿值累加。
  - 文本变化时不再重置整套语音 UI，首次显示在下一帧完成稳定定位。
  - 测试：显示端旋转回归 9/9、语音 UI/监听/会话/TTS/资源回归共 14/14 通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-30_显示端语音状态旋转与监视图动画.md`。

- ✅已完成 [2026-08-30][2026-08-30] 选择任务链接目标后保持面板打开
  - 点击“选择”发送链接请求后不关闭链接弹窗，支持连续选择多个目标显示端。
  - 测试：任务链接、链接面板和任务恢复回归 12/12 通过。
  - 文档：`docs/design/monitor-system.md`、`docs/spec/monitor-system.md`、`docs/task/2026-08-30_任务链接面板统一管理.md`。

- ✅已完成 [2026-08-30][2026-08-30] 调整显示端监听状态与柱状图为整体居中单行
  - 监听状态和柱状图横向排列且不换行，整体居中；最近识别文本放在下一行，继续适配四向旋转。
  - 测试：显示端旋转回归 9/9、语音聚焦回归 14/14，页面内联脚本编译和 `git diff --check` 通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-30_显示端语音状态旋转与监视图动画.md`。

- ✅已完成 [2026-08-30][2026-08-30] 解除任务链接后将运行中的目标移到添加列表
  - 解除操作即时将仍运行的目标移到当前弹窗的添加列表，弹窗保持打开并可立即重新链接；已停止目标只移除。
  - 测试：任务链接与链接面板回归 6/6 通过。
  - 文档：`docs/design/monitor-system.md`、`docs/spec/monitor-system.md`、`docs/task/2026-08-30_任务链接面板统一管理.md`。

- ✅已完成 [2026-08-30][2026-08-30] 修复任务链接面板单项解除误解除全部目标
  - 已链接目标节点补充目标实例 ID，解除时只删除当前目标；服务端拒绝缺少目标 ID 的请求。
  - 测试：任务链接与链接面板回归 6/6 通过。
  - 文档：`docs/spec/monitor-system.md`、`docs/task/2026-08-30_任务链接面板统一管理.md`。

- ✅已完成 [2026-08-30][2026-08-30] 调整显示端语音状态颜色与顶部音频柱状图
  - 监听中和暂停监听使用绿色、橙色明显区分；语音区域按状态、柱状图、最近识别文本顺序排列。
  - 柱状图宽度调整为 140px，0/90/180/270 度继续映射到逻辑顶部、右侧、底部、左侧中心。
  - 测试：显示端语音/旋转聚焦回归 14/14 通过，页面内联脚本编译和 `git diff --check` 通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-30_显示端语音状态旋转与监视图动画.md`。

- ✅已完成 [2026-08-30][2026-08-30] 统一任务链接面板管理多个显示端
  - 复用现有“链接到...”弹窗，展示来源实例已链接的目标任务、显示端和状态，并支持解除链接。
  - 一个来源实例可以链接多个目标实例，新增链接幂等；服务端过滤失效历史链接并去重。
  - 测试：任务链接、任务面板、任务恢复和渲染更新相关回归 13/13 通过；脚本语法检查和 `git diff --check` 通过。
  - 文档：`docs/design/monitor-system.md`、`docs/spec/monitor-system.md`、`docs/task/2026-08-30_任务链接面板统一管理.md`。
...
- ✅已完成 [2026-08-30][2026-08-30] 天气详情逐时数据只保留今日
  - 天气弹窗详情仅展开第一天预报的逐时天气，未来日期保留逐日汇总和天文信息，不展开逐时明细。
  - 结构化天气对象继续保留完整逐时数组，兼容其他调用方。
  - 测试：天气专项 6/6 通过。
  - 文档：`docs/design/display-control-refactor.md`、`docs/design/display-voice-conversation.md`、`docs/spec/voiceCommand.md`、`docs/task/2026-08-30_天气详情逐时仅今日.md`。

- ✅已完成 [2026-08-30][2026-08-30] 修复服务器重启后显示端监听不自动恢复
  - WebSocket 重连后重新检查 ASR，服务端 ASR 暂未就绪时每 5 秒重试；恢复后自动启动原本开启的监听。
  - WebSocket error 事件清理异常连接并进入统一重连流程，页面离开时取消相关定时器。
  - 测试：重连与语音相关聚焦测试 14/14 通过，`git diff --check` 通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-30_服务器重启后显示端监听自动恢复.md`。
...
- ✅已完成 [2026-08-29][2026-08-29] 浏览器显示端复用子显示端 VAD 分段缓存
  - VAD 触发前丢弃长静音，保留约 300ms 前置缓冲，并保持持续监听音频资源复用。
  - 测试：显示端 ASR、持续监听、VAD、唤醒/TTS 和共享录音入口回归 15/15 通过，脚本语法检查和 `git diff --check` 通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-29_显示端复用子显示端VAD分段缓存.md`。

- ✅已完成 [2026-08-29][2026-08-29] VAD 卡片只显示当前选中的显示端
  - 独立 VAD 卡片不再同时展示所有显示端，只针对当前选中的设备设置阈值和检测底噪。
  - 未选中或设备离线时显示提示，切换设备后自动刷新卡片内容。
  - 测试：VAD 布局、监听状态和相关脚本检查通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-29_显示端VAD当前设备卡片.md`。

- ✅已完成 [2026-08-29][2026-08-29] 将显示端 VAD 设置移至独立卡片
  - 设备列表保留监听控制和最近识别；VAD 阈值、底噪检测、统计结果和建议值移动到“显示控制”页独立卡片。
  - 保留按显示端配置、持久化和既有 WebSocket 协议，支持树形/列表设备视图。
  - 测试：VAD 配置布局契约、监听与语音回归通过，脚本语法检查和 `git diff --check` 通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-29_显示端VAD独立卡片布局.md`。

- ✅已完成 [2026-08-29][2026-08-29] 修复显示端持续监听 Chromium 内存泄漏
  - 复用同一个 PCM/VAD 音频采集链路，语音分段只清空当前 PCM 缓冲，不重复创建 `AudioContext`。
  - `ignored` 普通重试不再重启麦克风；连续无效冷却、关闭监听、异常和页面离开统一释放资源。
  - 测试：新增显示端资源生命周期回归，显示端监听/会话/内置指令/TTS/VAD 配置相关回归通过，脚本语法检查和 `git diff --check` 通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-29_显示端持续监听内存泄漏修复.md`。

- ✅已完成 [2026-08-29][2026-08-29] 增加显示端 VAD 设置与底噪检测
  - 设备列表可按显示端设置 VAD RMS 阈值；服务端按 `displayId` 持久化并在连接/修改时同步。
  - 增加约 3 秒静音采样，返回平均 RMS、峰值 RMS、P95 RMS、采样数和建议阈值，可直接应用建议值。
  - 检测期间不上传 ASR、不触发语音命令，控制端展示检测中、失败和完成状态。
  - 测试：VAD 契约测试、监听/会话/内置指令/TTS/服务端语音开关回归通过，脚本语法检查和 `git diff --check` 通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-29_显示端VAD配置与底噪检测.md`。

- ✅已完成 [2026-08-29][2026-08-29] 显示端语音输入 TTS 改走通用播放路由
  - 显示端语音命令、唤醒提示和语音触发的普通对话统一经过服务端 TTS fallback 和在线 `voicePlayback` 目标列表，不默认播放给输入显示端。
  - 控制端定向播放、`playOnControl` 和命令执行目标保持原有行为；停止播报按来源广播或定向发送。
  - 测试：显示端语音 TTS 路由、通用 TTS、显示端语音会话、监听、ASR 和服务端语音开关回归通过。
  - 文档：`docs/design/display-voice-conversation.md`、`docs/spec/display-voice-conversation.md`、`docs/task/2026-08-29_显示端语音输入TTS通用路由.md`。

- ✅已完成 [2026-08-29][2026-08-29] 增加控制端主题预览器
  - 系统设置的主题选择框旁增加“预览”按钮，弹窗展示典型卡片、按钮、开关、滑条、输入框、文本、Tip、状态、聊天、日志和弹窗样式。
  - 预览临时应用所选主题，关闭后恢复原主题，不保存浏览器或服务端配置；支持按钮、遮罩和 Esc 关闭。
  - 测试：主题预览契约、主题管理和显示端主题同步回归通过，脚本语法检查和 `git diff --check` 通过。
  - 文档：`docs/design/control-ui-theme.md`、`docs/spec/ui-theme.md`、`docs/task/2026-08-29_控制端主题预览器.md`。
...
- ✅ [2026-09-06] 恢复主服务器控制端聚合在线子服务器媒体库。
  - 控制端媒体库改用 AASC 网络媒体索引，主服务器和 Termux 媒体库统一展示。
  - 子服务器媒体库只读浏览和播放，文件 URL保持所属子服务器地址。
  - 显示端继续全部连接主服务器，不恢复显示端切换功能。
  - 验证：主服务器媒体索引包含子服务器媒体库和 `/storage` 目录；显示端统计保持在主服务器。

## AASC 主服务器远程媒体库管理

- ✅ [2026-09-07] 支持主控制端完整管理在线子服务器媒体库，并采用子服务器直连播放优先策略。
  - 新增 `media.library.*` 子服务器节点命令，支持添加、编辑、删除、设默认、文件删除、目录管理和远程批量播放。
  - 新增主服务器远程媒体库管理接口；配置持久化在目标子服务器，主服务器不保存远程账号密码。
  - 新增主服务器同源媒体上传/读取代理，转发视频 Range 响应；控制端和显示端优先使用子服务器 `directUrl`，失败时回退 `proxyUrl`。
