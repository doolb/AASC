...
test('显示端媒体底色应固定为黑色，UI 控件仍使用共享主题变量', () => {
    ...
    assert.match(timeDisplay, /text-shadow:\s*2px\s+2px\s+0\s+var\(--accent-color/u);
    assert.match(timeDisplay, /text-shadow:\s*2px\s+2px\s+0\s+color-mix\(in\s+srgb\s*,\s*var\(--accent-color/u);
    ...
    assert.match(fileNameDisplay, /text-shadow:\s*2px\s+2px\s+0\s+var\(--accent-color/u);
    assert.match(fileNameDisplay, /text-shadow:\s*2px\s+2px\s+0\s+color-mix\(in\s+srgb\s*,\s*var\(--accent-color/u);
