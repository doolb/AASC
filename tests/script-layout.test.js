...
test('test cases stay outside scripts and executable scripts use the unified taxonomy', () => {
    ...
    assert.equal(exists('scripts/api-sh/run-all.sh'), true);
    assert.equal(exists('scripts/api-sh/api-request.sh'), true);
    ...
    assert.equal(packageJson.scripts['api:test'], 'bash scripts/api-sh/run-all.sh');
