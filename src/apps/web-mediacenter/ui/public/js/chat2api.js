(function() {
  ...
  const Chat2APIControl = {
    ...
    async refresh() {
      try {
        const [config, providers, accounts, apiKeys] = await Promise.all([
          this.request('/api/chat2api/config'), this.request('/api/chat2api/providers'),
          this.request('/api/chat2api/accounts'), this.request('/api/chat2api/api-keys'),
        ]);
        this.render({ config, providers, accounts, apiKeys });
        this.message('');
      } catch (error) {
    ...
    render(data) {
      ...
      const accountRows = accounts.length ? accounts.map((account) => `<div style="display:flex;justify-content:space-between;gap:8px;padding:8px 0;border-bottom:1px solid rgba(255,255,255,.08)"><span>${this.escapeHtml(account.label || account.accountId)} <small style="color:#9aa7bf">${this.escapeHtml(account.providerId)}</small></span><span style="color:#8ee6a2">${account.enabled === false ? '已禁用' : '已登录'}</span></div>`).join('') : '<div style="color:#9aa7bf">暂无已登录账号</div>';
      const keyRows = apiKeys.length ? apiKeys.map((key) => `<div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid rgba(255,255,255,.08)"><span>${this.escapeHtml(key.label || key.id)}</span><code>${this.escapeHtml(key.maskedValue || '')}</code></div>`).join('') : '<div style="color:#9aa7bf">暂无 API Key</div>';
