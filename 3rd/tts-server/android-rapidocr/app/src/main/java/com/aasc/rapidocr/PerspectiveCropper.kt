...
object PerspectiveCropper {
    fun crop(bitmap: Bitmap, box: List<OcrPoint>): Bitmap {
        ...
        val targetPoints = MatOfPoint2f(
            Point(0.0, 0.0),
            Point(width.toDouble() - 1.0, 0.0),
            Point(width.toDouble() - 1.0, height.toDouble() - 1.0),
            Point(0.0, height.toDouble() - 1.0)
        )
        return try {
            ...
            try {
                ...
                Utils.matToBitmap(transformed, output)
                output
            } finally {
                perspective.release()
            }
