...
- ✅ 2026-09-02 真机当前大核/小核五模型测速（同一 `bus.jpg`，预热 1 次、正式 2 次）：大核 n/s/m/l/x 平均总耗时 `461.5/1306.0/4042.0/5277.5/11757.5 ms`，FPS `2.167/0.766/0.247/0.189/0.085`；小核平均总耗时 `1066.5/2992.0/8508.0/11086.5/23917.0 ms`，FPS `0.938/0.334/0.118/0.090/0.042`。
- ✅ 上述当前实测中，大核模式正式样本总推理耗时约 `22.845 s`，小核约 `47.570 s`；大核约为小核的 `2.03~2.31` 倍速度。模型加载时间另计，大核五模型合计约 `1.643 s`，小核约 `2.631 s`。
- ✅ 线程消耗核对通过：大核模式将 ORT 推理线程限制在 CPU `4-7`，小核模式限制在 CPU `0-3`；两种多核模式均配置 `intraOp=2`、`interOp=1`，实际最多使用 2 个同类推理线程，不会同时占满 4 个同类核心。
