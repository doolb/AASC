...
- 设备为 Android 9 arm64 `192.168.1.6:5555`，图片为同一张 `bus.jpg`，五模型串行执行，预热 1 次、正式运行 2 次；`averageTotalMs` 包含预处理、ORT 推理和后处理，不包含模型加载。
- 大核集合模式（CPU `4-7`，ORT `intraOp=2`）n/s/m/l/x 平均总耗时为 `461.5/1306.0/4042.0/5277.5/11757.5 ms`，FPS 为 `2.167/0.766/0.247/0.189/0.085`。
- 小核集合模式（CPU `0-3`，ORT `intraOp=2`）n/s/m/l/x 平均总耗时为 `1066.5/2992.0/8508.0/11086.5/23917.0 ms`，FPS 为 `0.938/0.334/0.118/0.090/0.042`。
- 真正单大核模式（1 个大核，ORT `intraOp=1`）n/s/m/l/x 平均总耗时为 `1368.0/3997.0/12190.0/15533.0/34630.0 ms`，FPS 为 `0.731/0.250/0.082/0.064/0.029`。
- 真正单小核模式（1 个小核，ORT `intraOp=1`）n/s/m/l/x 平均总耗时为 `1945.0/5530.5/16874.0/21607.0/46533.5 ms`，FPS 为 `0.514/0.181/0.059/0.046/0.021`。
- 五模型一轮正式样本合计约为大核集合 `22.845 s`、小核集合 `47.570 s`、单大核 `67.718 s`、单小核 `92.490 s`；单核心模式实际只使用 1 个 ORT 推理线程。
