...
/** 将任意图片转换为 YOLO11 固定的 640x640 RGB NCHW 输入。 */
object YoloPreprocessor {
    ...
    fun calculateTransform(sourceWidth: Int, sourceHeight: Int): LetterboxTransform {
        ...
        return LetterboxTransform(
            ...
            padLeft = (INPUT_SIZE - resizedWidth) / 2f,
            padTop = (INPUT_SIZE - resizedHeight) / 2f
        )
    }
    ...
    fun prepare(bitmap: Bitmap): PreparedInput {
        ...
        try {
            ...
            val destination = Rect(
                transform.padLeft.toInt(),
                transform.padTop.toInt(),
            ...
            return PreparedInput(toNchw(pixels, INPUT_SIZE, INPUT_SIZE), transform)
        } finally {
    ...
    fun toNchw(pixels: IntArray, width: Int, height: Int): FloatArray {
        ...
        pixels.forEachIndexed { index, pixel ->
            // 直接按 ARGB 位布局读取通道，既减少 Android Color 调用，也便于 JVM 单测运行。
