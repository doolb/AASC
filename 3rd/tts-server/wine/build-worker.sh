...
SDK_ROOT="${SDK_ROOT:-/mnt/AASC/3rd/NaturalVoiceSAPIAdapter/nvsapi-linux-poc/tmp/wine-probe/sdk-win41}"
...
x86_64-w64-mingw32-g++ \
  ...
  -o "$ROOT/bin/worker_tts_windows.exe" \
...
echo "built $ROOT/bin/worker_tts_windows.exe"

MINGW_BIN_DIR="$(cd "$(dirname "$(command -v x86_64-w64-mingw32-g++)")/../x86_64-w64-mingw32/bin" && pwd)"
cp -f "$MINGW_BIN_DIR/libwinpthread-1.dll" "$ROOT/bin/libwinpthread-1.dll"
echo "staged $ROOT/bin/libwinpthread-1.dll"
