...
describe('AdbClient', () => {
    ...
    it('starts an explicit activity on the selected display', async () => {
        const calls = [];
        const client = new AdbClient({
            serial: '192.168.1.6:5555',
            runner: async (file, args) => {
        ...
        await client.startActivity('com.netease.android.cloudgame', '.activity.SplashActivity', 0);
        expect(calls).toEqual([{
                file: 'adb',
                args: [
                    '-s', '192.168.1.6:5555', 'shell', 'am', 'start', '--display', '0',
                    '-n', 'com.netease.android.cloudgame/.activity.SplashActivity',
                ],
            }]);
    });
    it('uses the explicit serial for swipe movement', async () => {
        const calls = [];
        const client = new AdbClient({
            serial: '192.168.1.6:5555',
            runner: async (file, args) => {
        ...
        await client.swipe(100, 200, 150, 200, 400);
        expect(calls).toEqual([{
                file: 'adb',
                args: ['-s', '192.168.1.6:5555', 'shell', 'input', 'swipe', '100', '200', '150', '200', '400'],
            }]);
    });
