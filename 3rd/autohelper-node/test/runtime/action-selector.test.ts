...
const result = (score: number, rect = { x: 10, y: 20, width: 40, height: 80 }): MatchResult => ({
  score,
  rect,
  matched: true,
  method: 'template',
...
const candidate = (name: string, queue: number, score: number, options: Partial<ImageDescriptor> = {}): MatchCandidate => ({
...
describe('action selector', () => {
  ...
  it('calculates normalized and center click points inside the frame', () => {
    expect(resolveClickPoint(
      descriptor('button', 0, { clickPoint: { x: 0.75, y: 0.25 } }),
      { x: 10, y: 20, width: 40, height: 80 },
      { width: 100, height: 100 },
    )).toEqual({ x: 40, y: 40 });
    expect(resolveClickPoint(
      descriptor('center', 0, { centerClick: true }),
      { x: 10, y: 20, width: 40, height: 80 },
      { width: 100, height: 100 },
    )).toEqual({ x: 50, y: 50 });
  });
