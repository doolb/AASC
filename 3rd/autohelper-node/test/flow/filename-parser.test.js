...
describe('parseImageDescriptor', () => {
    it('parses goto and legacy click options from one image stem', () => {
        const descriptor = parseImageDescriptor('launch', '/tmp/enter@0.9,clickpoint@0.6&0.5,delay@800,goto@home.png');
    ...
    it('keeps negative queue and legacy flags', () => {
        const descriptor = parseImageDescriptor('home', '/tmp/a@0.8,~10@0.7,default,loop,wait,clickpoint_ab@0&0.png');
    ...
    it('parses OCR text as an additional image condition', () => {
        const descriptor = parseImageDescriptor('launch', '/tmp/enter@0.9,ocr@放弃福利,clickpoint@0.5&0.5,goto@home.png');
    ...
    it('parses a state match subgoal and its completion state', () => {
        const descriptor = parseImageDescriptor('task-panel', '/tmp/daily-row@0.9,match@daily-task,done@daily-task-complete,goto@execute-daily.png');
        expect(descriptor.matchId).toBe('daily-task');
        expect(descriptor.doneState).toBe('daily-task-complete');
        expect(descriptor.gotoFlow).toBe('execute-daily');
    });
});
