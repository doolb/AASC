...
const createDescriptor = (filePath: string): ImageDescriptor => ({
  name: 'button@0.95',
  flowId: 'home',
  filePath,
  queue: 0,
...
const createPattern = (width: number, height: number): Buffer => {
...
const embedPattern = (): { frame: Buffer; template: Buffer } => {
...
const createSolidImage = (width: number, height: number, value: number): Buffer => {
  const image = new PNG({ width, height });
...
describe('OpenCvImageMatcher', () => {
  ...
  it('does not expose a click rectangle when the score is below threshold', async () => {
    ...
    const result = await new OpenCvImageMatcher().match(
      createSolidImage(16, 12, 18),
      loadedTemplate,
    );
