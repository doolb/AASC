...
import { getOpenCv } from './opencv-runtime.js';
const noMatch = (method) => ({
    score: 0,
    rect: null,
    matched: false,
    method,
});
const toGrayMat = (cv, image) => {
...
const templateMatch = (cv, frame, template, threshold) => {
 ...
 */
const orbMatch = (cv, frame, template, threshold) => {
...
export class OpenCvImageMatcher {
    templateCache = new Map();
    async match(frameBuffer, template, method = 'template') {
    ...
    async matchAll(frameBuffer, templates, method = 'template') {
        const cv = await getOpenCv();
        const frameImage = decodeImage(frameBuffer, 'adb-screenshot.png');
        const frameMat = toGrayMat(cv, frameImage);
        try {
            return await Promise.all(templates.map(async (template) => (await this.matchWithFrameMat(cv, frameMat, template, method))));
        }
        finally {
            frameMat.delete();
        }
    ...
    async matchWithFrameMat(cv, frameMat, template, method) {
