...
const readResponse = (response) => (new Promise((resolvePromise, reject) => {
    const chunks = [];
...
const finishRequest = (clientRequest, body, options, resolvePromise, reject) => {
...
const requestJson = async (urlString, body, options) => {
...
const parseBox = (value) => {
    ...
    const points = Array.isArray(value.points)
        ? value.points.map(parsePoint).filter((point) => point !== null)
        : [];
...
const parseOptionalSize = (value) => (typeof value === 'number' && Number.isInteger(value) && value > 0 ? value : undefined);
const responseError = (payload, statusCode) => {
...
export class OcrClient {
    ...
    async recognize(frame) {
        const requestBody = { imageBase64: frame.toString('base64') };
        ...
        const boxes = Array.isArray(payload.boxes)
            ? payload.boxes.map(parseBox).filter((box) => box !== null)
            : [];
        return {
            boxes,
