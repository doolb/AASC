...
function parseFiniteNumber(value, field, filePath) {
...
function parsePoint(value, field, filePath) {
...
export function parseImageDescriptor(flowId, filePath) {
    ...
    for (const token of stem.split(',')) {
        ...
        switch (key) {
            case 'match':
                if (!value || !FLOW_ID_PATTERN.test(value)) {
                    throw new Error(`invalid match id in image filename: ${filePath}`);
                }
                descriptor.matchId = value;
                break;
            case 'done':
                descriptor.doneState = validateGotoFlow(value, filePath);
                break;
            case 'ocr':
                if (!value.trim()) {
