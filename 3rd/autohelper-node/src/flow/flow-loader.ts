...
import type { FlowContext, ImageDescriptor, LoadedTemplate } from '../types.js';
import type { TemplateLoader } from '../vision/template-cache.js';

const IMAGE_EXTENSIONS = new Set(['.png', '.bmp']);
const FLOW_ID_PATTERN = /^[A-Za-z0-9._-]+$/;

export type FlowLoaderOptions = {
  flowsRoot: string;
  parseDescriptor?: (flowId: string, filePath: string) => ImageDescriptor;
...
export class FlowLoader {
  private readonly flowsRoot: string;
  private readonly parseDescriptor: (flowId: string, filePath: string) => ImageDescriptor;
  private readonly templateCache: TemplateCache;
  private activeContext: FlowContext | null = null;

  public constructor(options: FlowLoaderOptions) {
  ...
  public async load(flowId: string): Promise<FlowContext> {
    ...
    const context: FlowContext = { id: flowId, directory, descriptors, templates };
  ...
  private resolveFlowDirectory(flowId: string): string {
  ...
  private async assertFlowDirectory(flowId: string, directory: string): Promise<void> {
    try {
