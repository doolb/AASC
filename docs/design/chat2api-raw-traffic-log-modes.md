...
为 Chat2API 原始流量调试日志增加“完整”和“简洁”两种模式。完整模式保持现有排障能力；简洁模式只输出 Provider 请求的模型、文本和最终输出，不输出 URL、请求头、Cookie、Token、原始响应体或逐块流量。
...
Chat2API 配置 rawTrafficMode
    └─ Provider adapter
        └─ Raw traffic logger
            ├─ full: 原始请求/响应/流块
            └─ compact: model + text -> output
```

## 验收标准

- 默认仍为完整模式，已有测试和日志格式不变。
- 简洁模式日志数据只包含请求 `model/text` 和响应 `output`。
