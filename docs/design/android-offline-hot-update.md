...
新增外网资源同步命令，用于在家庭或公司机器上把当前外网清单中的服务资源同步到指定本地 HTTP 根目录。同步目标支持 `--local-root` 和 `AASC_OFFLINE_LOCAL_ROOT` 覆盖，默认保持 `/mnt/aasc-offline`。同步代码包、生产依赖包、min APK 和数据修复包；完整 APK 不进入同步范围。默认必须加载公钥并验证清单 RSA 签名。公钥不可用时，可显式传入 `--skip-signature-verification` 跳过本地 RSA 验签；该参数不跳过清单结构、资源白名单、路径安全、文件大小和 SHA-256 校验，且命令须警告此模式无法认证资源来源，清单中的哈希只用于校验下载内容是否与该清单自洽。同步前先检查本地目标：文件大小和 SHA-256 与清单一致时标记为已复用并跳过网络下载；同版本内容冲突时在下载前报错；只有目标缺失时才下载。下载期间命令行显示当前相对文件名、单文件已传/总大小及百分比，并显示所有待同步资源按字节加权的总进度；交互终端刷新当前行，日志输出按进度节点节流。版本文件采用临时文件和原子清单替换，最后再清理严格匹配的过时版本文件。Android 客户端仍会验证签名，不会因同步参数而降低客户端校验要求。
