...
控制端状态卡片读取 `runtime.displayCount`，缺失时显示“未上报”；旧的 `priority` 不属于 AASC 节点状态，显示为“不适用”。媒体库数量为零时，服务器自己的 `/control` 页面仍显示“+ 添加”入口。主服务器共享媒体库摘要继续保持只读，暂不通过 AASC 远程创建或编辑子服务器媒体库。
