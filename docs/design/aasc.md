...
- 子显示端连接后会上报 `capabilities` 消息，声明自己是否具备媒体渲染、语音播放、语音录音、语音识别、文本显示能力。
- AASC 必须为 `capabilities` 提供固定路由，统一交给显示状态执行者更新 `displayList`。
- 语音输入 `voiceInput` 需要在 AASC 层做无效内容过滤和串行排队，避免同一显示端连续语音并发执行导致状态错乱。
...
AASC 网络中的任务默认优先交给具备对应能力的显示端执行。显示端可以直接处理媒体渲染、播放控制、截图、录音、ASR、TTS、OCR、YOLO 等与本机硬件或显示上下文相关的任务。

### 15.2 子服务器使用条件

子服务器是可选的服务型执行节点，不参与所有任务的默认转发。只有在以下场景出现时才使用子服务器：

1. 任务依赖服务器本地媒体库、NAS、数据库或长期持久化资源。
2. 任务依赖 Node.js、Puppeteer、后台脚本或其他不适合放在显示端的运行环境。
3. 任务需要在显示端离线后继续运行。
4. 任务需要独立的资源隔离、网络出口、聚合或多显示端网关。
...
收到任务
    ...
    → 查询在线显示端能力
    → 存在匹配显示端时，优先投递到显示端
    → 没有匹配显示端且任务允许服务端执行时，投递到主服务器或子服务器
    → 没有可用执行节点时，返回明确的能力不可用结果
```

主服务器负责 AASC 网络入口、节点注册、消息路由和媒体索引；子服务器只在能力或运行边界确有需要时加入。当前阶段不要求部署子服务器，也不要求将显示端任务强制经过子服务器。

## 16. AASC 网络演进顺序

第一阶段固定使用主服务器作为 AASC 网络入口，服务器地址由节点预先配置，例如 `https://192.168.1.39:8081/`。当前不实现局域网自动发现。

功能按以下顺序演进：

1. **服务器节点注册与心跳**：主服务器维护在线服务器节点，节点上报 `nodeId`、地址、版本和能力；控制端服务器页面展示真实节点状态。
2. **服务器代码下发**：通过 HTTP `/server` 提供版本清单、代码包和校验信息，由预装 Bootstrap 的节点自动检查、下载和启动更新。
3. **服务器媒体库索引**：各服务器保留本地文件，向 AASC 网络上报媒体索引，主服务器聚合索引，播放时访问媒体所属服务器。
4. **显示端任务路由**：任务优先投递到匹配的显示端；显示端无法处理时，再回退到主服务器或子服务器。
5. **网页版服务器切换**：控制端支持在已登记服务器之间手动切换，后续再评估断线切换。
6. **权限认证**：最后增加用户、节点、媒体库和操作权限。

以下能力暂不纳入当前阶段：自动发现、最近服务器算法、媒体文件全量同步、主服务器故障转移和权限认证。

## 17. 服务器节点注册与心跳（第一阶段）

### 17.1 目标

主服务器作为固定的 AASC 网络入口，维护服务器节点的临时在线目录。服务器节点启动后主动向主服务器登记，随后周期性发送心跳；控制端通过统一接口查看节点地址、能力、版本和在线状态。

### 17.2 节点信息

每个服务器节点使用调用方生成且持久化的 `nodeId` 标识自己，登记信息包括：

- `nodeId`：节点唯一标识，必填。
- `name`：控制端展示名称，可选。
- `url`：节点 HTTPS/HTTP 基础地址，必填。
- `version`：服务器代码版本，可选。
- `capabilities`：服务器能力键值表，可选。
- `metadata`：受控的附加信息，可选，例如角色和平台。
- `registeredAt`、`lastHeartbeatAt`：主服务器记录的时间。
- `status`、`healthy`：由主服务器根据心跳过期时间计算，不接受节点自行伪造。

### 17.3 HTTP 接口

| 方法 | 路径 | 用途 |
|------|------|------|
| POST | `/api/aasc/servers/register` | 注册或更新服务器节点，并立即标记为在线 |
| POST | `/api/aasc/servers/:nodeId/heartbeat` | 更新已登记节点的心跳及可变状态 |
| GET | `/api/aasc/servers` | 获取 AASC 节点目录 |

心跳超时暂定为 90 秒，节点通常每 30 秒发送一次。目录读取时惰性刷新过期状态为 `offline`，不删除节点记录，便于控制端观察和后续恢复。

主服务器使用固定 `nodeId=main-server` 登记自身。每次读取节点目录前，主服务器刷新自身心跳，确保当前正在提供目录接口的主服务不会因为没有外部心跳请求而被误判为离线；远程节点仍按照 90 秒超时规则计算状态。

### 17.4 兼容现有子服务器配置

旧的 `SubServerManager` 配置仍由 `/api/subservers` 管理，但不再注入 `/api/aasc/servers`。AASC 节点目录只接受主动 WebSocket 注册，避免主服务器继续按旧配置主动访问子服务器。

### 17.5 边界

本节点登记阶段不实现自动发现、注册鉴权、心跳签名、媒体索引同步和故障转移。服务器代码下发与 Termux 双进程热更新已在 `docs/design/android-termux-server.md` 中单独落地；正式节点通过 `/server` WebSocket 主动注册。

## 18. 服务器媒体库索引共享（首版）

当前实现状态：已完成本地索引、已登记在线节点聚合和控制端共享媒体库摘要；暂不做后台全量扫描、持久化索引、媒体同步和权限认证。

各服务器继续保存自己的媒体文件，不做媒体文件复制和全量同步。每台服务器通过本地索引服务生成媒体库元数据及目录索引；主服务器通过主动连接发送 `media.index.local` 请求，聚合在线节点返回的统一目录。

索引记录包含 `nodeId`、服务器地址、媒体库 ID/名称/类型/只读状态、当前目录和条目元数据。条目的实际播放地址仍指向所属服务器的 `/api/media-libraries/:id/proxy/*` 或现有媒体接口，主服务器只保存短期内存聚合结果，不代存文件。

远程节点不可达时保留本次请求中的其他来源并标记该来源错误；不删除节点登记，也不伪造远程索引。当前不实现后台全量扫描、索引持久化、媒体同步和权限认证。

## 19. 显示端优先任务路由（首版）

当前实现状态：已完成 `display-first/auto` 的显示端能力匹配和当前主服务器回退；子服务器已具备接收明确 `task.execute` 请求的本地执行入口，主服务器自动跨服务器任务选择暂不实现。

任务可声明 `routing: 'display-first'` 或 `target: 'auto'`，并通过 `requiredCapabilities` 声明所需显示端能力。路由器先检查在线显示端及能力覆盖情况；有候选时沿用现有 `task:execute` 转发协议，未找到候选时回退到当前服务器执行。显式 `target: 'display'/'subdisplay'` 和 `target: 'server'` 的旧任务行为保持不变。

首版回退节点是当前主服务器进程；子服务器节点目录只用于能力和地址展示，不新增跨服务器任务执行代理。这样可以先保证显示端优先和本机回退的确定性，跨服务器执行另行设计。

## 20. 网页版服务器手动切换（首版）

当前实现状态：已完成在线节点连接按钮、当前节点标识、共享媒体库摘要和合法 HTTP(S) 地址手动连接；不做自动切换。

控制端服务器页面在每个在线节点卡片提供“连接”按钮，跳转到该节点的 `/control`；当前服务器按钮显示“当前连接”。页面同时保留手动输入 `http/https` 服务器地址的连接入口，输入地址只允许合法协议和主机，自动补齐 `/control` 路径。

切换是浏览器页面级切换，不做后台代理、自动发现、最近节点选择或断线自动切换。服务器列表接口和既有控制端 WebSocket 在新页面中自动使用新页面的同源地址。

## 21. 子服务器主动连接架构（已实现，权限认证待后续）

本阶段将服务器节点连接方向调整为“子服务器主动连接主服务器”。主服务器固定使用 `https://192.168.1.39:8081/` 作为 AASC 网络入口，子服务器服务进程启动后主动连接 `wss://192.168.1.39:8081/server`，通过持久 WebSocket 完成注册、心跳、远程请求和命令响应。

`/server` 同时提供 HTTP 代码接口和 WebSocket 节点连接：HTTP `GET /server` 返回代码清单，HTTP `GET /server/package` 返回代码包，WebSocket `/server` 接收子服务器主动连接。主服务器不再使用子服务器的 `url` 发起健康检查或控制请求；该地址只用于显示端、网页和媒体实际访问。

节点配置增加 `aasc.role`、`aasc.mainServerUrl`、`aasc.nodeId`、`aasc.nodeName`、`aasc.advertisedUrl`、心跳间隔和重连退避参数。主服务器角色默认为 `main`，子服务器角色为 `subserver`。子服务器仍使用 `server-launcher.js` 和 `server-app.js` 两进程模型，更新命令由子服务器主动拉取 `/server` 并复用 Bootstrap 完成停止、热更、启动和回滚。

主服务器节点目录只展示主动连接注册表中的节点；旧 `/api/subservers` 保留兼容，但不再作为 AASC 节点的连接来源。远程媒体索引和明确指定服务节点的请求通过 WebSocket 请求，主服务器提供 HTTP `POST /api/aasc/servers/:nodeId/request` 作为控制/API 调用入口；显示端优先任务规则、网页手动输入地址、`/display` 和 `/control` 协议保持不变。

当前不实现权限认证、自动发现、最近节点算法、媒体文件同步、主服务器故障转移和普通任务强制迁移。

## 23. 控制端批量重载子服务器代码

控制端“服务端”设置区域提供“重载所有子服务端代码”按钮。按钮调用主服务器的批量 API，由主服务器从 AASC 主动连接注册表中筛选 `status=online` 且存在连接句柄的子服务器，并通过现有节点 WebSocket 请求发送 `server.update`。

批量更新不包含主服务器，不通过子服务器 `advertisedUrl` 发起 HTTP 更新。每个子服务器独立执行 Bootstrap 下载、校验、停止、替换、启动和回滚流程；主服务器汇总每个节点的已接受、失败或不可用结果，允许部分节点成功。当前按钮是手动触发，不实现自动版本检测和权限认证。

## 24. 控制端强制让子服务器获取最新代码

控制端“服务端”设置区域在普通批量重载按钮旁增加“强制获取最新代码”按钮。该按钮调用 `POST /api/aasc/servers/force-update-all`，主服务器沿用批量重载的节点筛选规则，仅向在线且存在主动 WebSocket 连接的非主服务器发送 `server.update`，并在 payload 中设置 `force=true`。

子服务器收到强制更新请求后，启动 Bootstrap 的 `update --force` 模式。Bootstrap 为 `/server` 清单和代码包请求附加一次性缓存绕过参数，保证即使版本号未变化也重新获取主服务器当前代码；随后仍执行大小、SHA-256、压缩包路径、备份、服务停止/启动、健康检查和失败回滚。控制端显示批量接受、失败或无在线节点结果，不改变自动发现、权限认证和自动更新边界。

## 22. 节点运行数据上报与 Termux 媒体库空状态

节点注册和心跳可以携带运行时统计 `runtime`，用于控制端展示节点当前承载情况：

- `displayCount`：当前服务器连接的显示端数量。
- `controlCount`：当前服务器连接的控制端数量。
- `libraryCount`：当前服务器已经初始化的媒体库数量。

主服务器和 Termux 子服务器都从本地运行时采集上述数据，通过 `/server` WebSocket 的 `node.register` 和 `node.heartbeat` 上报。主服务器注册表只保存最近一次节点上报值，不扫描远程文件，也不通过节点地址主动查询。

控制端状态卡片读取 `runtime.displayCount`，缺失时显示“未上报”；旧的 `priority` 不属于 AASC 节点状态，显示为“不适用”。媒体库数量为零时，服务器自己的 `/control` 页面仍显示“+ 添加”入口。主服务器控制端可以通过 AASC 节点连接创建和管理子服务器媒体库，配置只持久化在目标子服务器。
...
## 25. 主服务器控制端聚合与管理子服务器媒体库（显示端固定主服务器）

主服务器控制端继续作为唯一入口，媒体库面板通过 `/api/aasc/media-index` 读取主服务器和在线子服务器的网络索引。显示端不参与本次媒体库聚合，仍全部连接主服务器 AASC。

- 主服务器媒体库保持原有库 ID，可上传、删除和编辑。
- 子服务器媒体库在控制端使用节点 ID 与原始媒体库 ID分离保存，不能把复合展示 ID直接发送到本地媒体库 API。
- 主控制端可以选择在线子服务器添加媒体库；编辑、删除、设默认、文件上传、文件删除和目录操作由主服务器转发到目标子服务器。
- 配置和小型操作通过子服务器主动建立的 `/server` WebSocket 下发；媒体上传和读取通过主服务器同源 HTTP 代理转发，避免浏览器直接跨节点访问。
- 播放时优先使用子服务器返回的原始媒体 URL，控制端或显示端无法直连时回退到主服务器代理 URL；索引中同时保留两种地址。
- 远程媒体直连地址以 AASC 节点注册的 `node.url` 为权威主机，保留条目路径；子服务器返回地址为空、`null`、`undefined` 或不可解析时，按原始媒体库 ID 和文件路径生成子服务器媒体 API 地址，避免把无效地址下发为 `null`。
- 远程媒体库批量播放沿用同一地址规则：子服务器播放列表必须携带文件 `path`，主服务器按注册节点地址重写直连 URL，并附带主服务器远程代理回退地址。
- 远程播放控制消息携带来源节点标识；主服务器收到子服务器首次注册，或心跳中的 `node.url` 发生变化时，扫描当前显示端播放状态，按当前媒体/播放列表的原有消息类型重发一次正常播放消息。
- 正常重发只复用既有的 `url`、`base64` 和 `playlistStart` 流程，不新增显示端协议；主服务器按最新 `node.url` 重写该节点的直连地址并保留代理回退地址，其他节点的播放状态不受影响。
- 所有仍在播放该子服务器媒体的显示端都会被处理，包括未发起播放的其他显示端；显示端不需要识别节点就绪事件、重连事件或 IP 变化。
- 当直连地址和主服务器代理地址都不可用时，控制端不生成 `src`/播放请求，并显示媒体地址不可用提示。
- 子服务器离线或索引失败时，主服务器保留本地媒体库并显示节点错误，不影响其他节点。
- 当前不实现权限认证、自动发现、最近节点算法、媒体文件同步、主服务器故障转移和普通任务强制迁移。
- 本功能不改变显示端固定连接主服务器的地址和显示消息路由。
