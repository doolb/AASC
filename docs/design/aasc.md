...
旧的 `SubServerManager` 配置仍由 `/api/subservers` 管理。`/api/aasc/servers` 首阶段同时返回 AASC 动态登记节点和未重复的旧配置节点，避免控制端迁移期间丢失已配置服务器；旧节点的在线状态仍来自原有健康检查。

### 17.5 边界

本节点登记阶段不实现自动发现、注册鉴权、心跳签名、媒体索引同步和任务路由。服务器代码下发与 Termux 双进程热更新已在 `docs/design/android-termux-server.md` 中单独落地；Termux 节点主动注册客户端仍属于后续阶段。

## 18. 服务器媒体库索引共享（首版）

当前实现状态：已完成本地索引、已登记在线节点聚合和控制端共享媒体库摘要；暂不做后台全量扫描、持久化索引、媒体同步和权限认证。

各服务器继续保存自己的媒体文件，不做媒体文件复制和全量同步。每台服务器通过 `GET /api/aasc/media-index?scope=local&path=/` 暴露本地媒体库元数据及目录索引；主服务器通过已登记的 AASC 节点地址读取在线节点的相同接口，聚合为统一目录。
...
当前实现状态：已完成 `display-first/auto` 的显示端能力匹配和当前主服务器回退；显式目标兼容，跨服务器任务代理暂不实现。
...
控制端服务器页面在每个在线节点卡片提供“连接”按钮，跳转到该节点的 `/upload`；当前服务器按钮显示“当前连接”。页面同时保留手动输入 `http/https` 服务器地址的连接入口，输入地址只允许合法协议和主机，自动补齐 `/upload` 路径。
...
`/server` 按协议区分职责：HTTP `GET /server` 返回代码清单，HTTP `GET /server/package` 返回代码包，WebSocket `/server` 接收子服务器连接。主服务器不再使用子服务器的 `url` 发起健康检查或控制请求；该地址只用于显示端、网页和媒体实际访问。
...
主服务器节点目录只展示主动连接注册表中的节点；旧 `/api/subservers` 保留兼容，但不再作为 AASC 节点的连接来源。远程媒体索引和明确指定服务节点的任务通过 WebSocket 请求，显示端优先任务规则、网页手动输入地址、`/display` 和 `/control` 协议保持不变。
