...
- 设备名仍作为 CPU 条的标签位（固定宽 `deviceNameW`，明确使用 `text-align:left` 左对齐，避免短名称如 `arch0` 因右对齐产生视觉前导空白）。
...
设备名、`M`、`GPU` 和 `VRAM` 等进度条外标签使用固定白色文字与固定黑色阴影，避免主题文字色影响监控覆盖层的可读性；同时增加正下方 2px、无模糊的主题强调色投影。投影使用约 65% 不透明度的主题色，先提供纯色回退以兼容不支持 `color-mix()` 的 Chromium/WebView；不使用文字轮廓或元素盒子的内阴影，避免出现矩形框。
