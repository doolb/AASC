...
普通聊天的每个 LLM profile 增加独立调用模式。`llm` 模式继续直接调用 profile 配置的 OpenAI 兼容接口；`agent` 模式支持 Pi 或 Codex，由服务器进程分别管理同进程 Pi SDK AgentSession 或 Codex app-server 子进程。
...
- 每个 profile 按聊天会话管理对应 AgentSession 或 Codex 进程和内存会话：群聊共用一个，私聊按助手角色独立；模式或角色切换时回收旧会话，下一次请求重新创建运行时。
...
- profile 之间的聊天历史和 Agent 会话隔离；工作 AI 角色的历史和运行时不参与普通 LLM Agent。
...
- 服务器负责 Pi SDK AgentSession 的创建、事件订阅、队列串行化、异常回收、配置变更重建和服务器退出 dispose。
...
- Pi Agent 请求记录 requestId 生命周期日志，覆盖排队、启动、收到关键 SDK 事件、完成、失败和超时；错误型空 `agent_end` 必须作为失败回传，不能伪装成空成功回复。
- 控制端聊天输入和 `chatResponse` 记录同一 requestId，便于区分 Pi 未返回、服务器处理失败和前端丢弃迟到回包。
- Pi 请求增加独立排队超时，避免前一个请求异常时后续请求无限等待；空回复、SDK 错误或协议错误会 dispose 当前会话，空回复/可恢复 SDK 错误最多自动用新会话重试一次。
...
server-app -> llm-service 当前 profile
    ...
    │                              └─ @earendil-works/pi-coding-agent AgentSession（同一 Node 进程，固定只读工具）
    └─ mode=agent/backend=codex -> CodexRuntimeManager
                                   └─ codex app-server --stdio（服务器子进程，独立 threadId + 只读沙箱）
```

通用 `PiRuntimeManager` 在服务器进程内维护 `(profileName, templateId, permissionProfile, conversation) -> AgentSession` 映射，聊天只是第一个调用方。后续服务器内置命令可以复用 Runtime，但必须使用自己的上下文和工具策略。Pi 不启动 CLI 子进程，服务器退出时逐个 abort/dispose SDK 会话。单个 session 的并发请求串行化，避免上下文和响应互相污染。单次 Pi SDK 请求默认使用 600 秒超时；测试或特殊调用可以注入更短的超时，超时后 abort 并 dispose 当前会话，再由下一次请求重建。排队等待默认最多 30 秒，排队超时不调用 `session.prompt()`。空回复或可恢复 SDK 错误会先 dispose 当前 session，再自动重试一次；重试仍失败才通知上层。

## Pi 权限边界

Pi SDK 创建 AgentSession 时固定使用以下约束：

- `ModelRuntime.registerNativeProvider()` 注册内置 `aasc-openai` Provider，API Key 由当前 profile 闭包传入，不通过进程环境或命令行传递。
- `DefaultResourceLoader` 同时禁用 extensions、skills、prompt templates、themes 和 context files，不读取项目 `AGENTS.md`/`CLAUDE.md` 等上下文文件。
- `SessionManager.inMemory()` 和 `SettingsManager.inMemory()` 禁止 Pi 会话持久化到磁盘。
- `tools` 固定为 `read,grep,aasc_find,ls,aasc_web_search,aasc_web_fetch`；不启用 Pi 原生依赖 `fd` 的 `find`，避免工具首次运行时下载失败。
- `customTools` 只注册 `aasc_find`、`aasc_web_search` 和 `aasc_web_fetch`，不开放 `bash`、`edit`、`write`。
- 工作目录固定为项目根目录，权限策略由服务器根据模板枚举映射，不接受请求体动态传入工具集合。
...
首次 Agent 会话初始化时包含当前 profile 的系统提示词、角色模板和当前聊天会话的必要历史；重建文本统一使用 `User:`/`Assistant:` 标记。Pi 初始 prompt 包含完整重放文本，后续只发送带 `User:` 前缀的当前用户消息；Codex 初始 thread 使用 developer instructions 接收系统提示词，首轮 turn 接收历史和当前消息，后续 turn 只接收当前消息。Pi 的上下文由同进程 AgentSession 保留，Codex 的上下文由独立 thread 保留，工具调用结果不写入普通聊天历史。

Pi AgentSession 自带上下文压缩能力；当前 `PiRuntimeManager` 暂不增加控制端主动压缩入口，仍由 SDK 在接近上下文上限时按自身策略自动处理。后续若启用主动压缩，需要保证压缩请求与当前聊天 session 串行，并向控制端回报压缩状态。
...
- Pi SDK/Codex provider 配置失败、SDK 事件错误、超时或异常退出：当前请求返回失败消息，并清理该 profile 的 AgentSession 或进程。
- Agent 模式失败不回退为直接 LLM 请求，避免绕过只读权限和用户选择的执行模式。
- 一个 profile 的 Agent 失败不影响其他 profile 或普通 LLM 请求。
- 服务器关闭时 dispose 全部 Pi AgentSession 并清理 Codex 进程；清理失败记录日志，但不阻塞服务器退出。
- Chat2API 工具标签格式错误、参数无法解析或工具名不在只读白名单时，禁止执行；如果响应中存在协议错误前的安全普通文本，则回退为普通助手回复并记录错误，不把原始协议标签、工具 JSON 或残留标签输出给聊天/TTS；没有可保留文本时才返回明确的 Agent 错误。
- Pi 返回 `agent_end` 时，如果最后助手消息为错误停止原因、携带错误消息或最终文本为空，当前请求返回失败；`willRetry` 事件不提前结束仍可重试的请求。
- 请求失败时当前 Pi session 立即 dispose，避免异常上下文被后续请求复用；排队超时直接失败并释放排队任务，不占用 Pi 并发队列。空回复和可恢复 SDK 错误自动新建 session 重试一次，超时、协议错误和配置错误不盲目重试。
...
当上游模型只返回普通文本、半截 JSON 工具数组或残留 Chat2API 结束标签时，Provider 层保留第一个协议异常位置之前的普通文本。该回退只用于展示和播报，不创建任何 `ToolCall`，不执行部分调用，也不把不完整的搜索承诺视为搜索成功；没有安全前缀时沿用失败回报。
...
- `mode=agent` 不调用 `apiUrl` 的普通聊天路径，而是在服务器 Node 进程内创建对应 profile 的 Pi SDK AgentSession。
...
- Pi 失败后前端收到失败回包，服务器可在下一次请求重新创建该 profile 的 AgentSession。
- 服务器退出后不存在由本功能遗留的 Pi AgentSession；Codex 进程仍按原有生命周期清理。
