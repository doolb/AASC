...
显示页因旧版 WebView 原生 ASR/TTS 同步调用而暂时阻塞 JavaScript 时，旧连接的 `close` 回调和已有重连定时器可能交错执行。新版 APK 的 ASR/TTS 使用异步原生桥，推理结果通过 JS 回调返回；客户端仍必须保证同一页面最多存在一个重连定时器和一个有效 WebSocket，旧 socket 的事件不得改变新 socket 的状态。页面进入 `pagehide` 后取消重连并关闭当前 socket，避免页面刷新或销毁后继续创建新连接。

Node 子显示端沿用网页显示端的固定 3 秒重试间隔，网络不可达时持续重试，不因连续失败退出进程。`error` 与 `close` 共用幂等清理入口，每次至多保留一个重连定时器和一个连接尝试；异常连接被关闭，旧 socket 事件不得更改新连接状态。重连始终复用进程启动时生成的本地 `displayId`，服务器确认 ID 单独保存，不反写覆盖本地身份。断线立即停止并释放麦克风，重连后重新获取服务端确认的 `displayId` 和权威能力值；服务端确认录音能力开启前保持麦克风关闭。进程正常停止时取消重连定时器。
...
## 显示端UI旋转
 ...
 - ✅已完成 [2026-08-26][2026-08-26] 修正90°/270°旋转文本上下位置偏移
   ...
   - 实现文档：docs/spec/display-ui-rotation.md
 - ✅已完成 [2026-08-26][2026-08-26] 媒体名字号放大一倍
   - `fileNameDisplay` 桌面端字号由 24px 调整为 48px，移动端由 16px 调整为 32px。
   - 旋转布局继续读取放大后的实际包围盒；0°覆盖层保持在媒体名上侧，90°/180°/270°继续分别从右侧/下侧/左侧避让，避免媒体名越界或与 render-display 覆盖层重叠。
   - 实现文档：docs/spec/display-ui-rotation.md
...
Android DeX 多屏场景下，原生无障碍输入注入可能固定作用于主显示屏，不能可靠地把触摸/滚轮事件投递到 HDMI 目标显示。因此显示端控制模式保留分层策略：键盘和文本输入使用 `NativeDisplay.injectKey/injectText`，触摸和滚轮使用页面内 JavaScript 合成事件；无原生桥时所有输入均继续使用浏览器现有回退链。该策略由 `tests/display-native-bridge.test.js` 验证。
