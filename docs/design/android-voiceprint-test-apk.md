...
蓝牙 SCO 修复状态：已完成 `BluetoothScoController`、8 kHz 采集到 16 kHz 重采样、录音状态校验和失败提示；每次蓝牙录音都会清理旧 SCO 状态并重新建链。Debug APK 已覆盖安装到 SM-N9500，设备 ID 922 已现场确认录音有声音。

## 原生 ASR 蓝牙 SCO 无数据修复（2026-09-20）

- 已确认测试 APK 的 SCO 控制器与正式 APK 不一致：测试 APK 等待到 SCO CONNECTED 后没有设置 `AudioManager.isBluetoothScoOn=true`，导致 `AudioRecord` 可能启动但实际没有蓝牙输入数据；失败后 MainActivity 又自动回退到了系统默认内置麦克风。
- 测试 APK 现在保存录音前的 SCO 开关状态，连接成功后同步打开 SCO 开关，并在停止时仅恢复由测试 APK 修改的标志和音频模式。
- SCO 状态接收器将 `ERROR`、`DISCONNECTED` 和超时统一视为失败，避免把未建立的蓝牙链路交给 `AudioRecord`。

## 原生 ASR 内置麦克风无声诊断修复（2026-09-20）

- 针对 SM-N9500 暴露的两个内置输入源，主内置麦克风（设备 ID 10、地址 `bottom`）显示为“内置主麦克风（底部）”，后置输入源（设备 ID 11、地址 `back`）显示为“内置后置麦克风”，并在名称中显示设备 ID，避免现场无法区分多个麦克风。
- 首次运行优先选择主内置麦克风；用户已保存的设备选择继续优先，设备不可用时才回退。
- 原生录音使用 `READ_BLOCKING` 读取，录音结束前读取 `AudioRecord` 实际路由设备，并统计样本数、PCM16 峰值和 RMS。
- 当录音线程正常运行但 PCM 全零时，页面明确提示“没有有效声音”并展示实际路由和音量统计，用于区分路由成功但无输入、路由错误和录音启动失败。
...
## 三个 Sherpa 声纹模型对比测试（2026-09-12）

独立 ASR 测试 APK 仅增加声纹模型对比，不更换或增加任何 ASR、流式 ASR、降噪和分割模型。声纹 embedding 模型提供三个选项：现有 `ERes2Net-base`、`ERes2Net-large` 和 `ERes2NetV2`。

- 原生 APK 页面和内置测试网页均提供同一组三选一模型选项，并显示当前模型名称、embedding 维度、匹配分数和阶段耗时。
- 同一时间只加载一个 embedding 模型；切换模型时释放旧 Sherpa native 引擎并清空进程内已注册声纹，要求切换后重新注册，避免不同模型的 embedding 混用。
- 三个 embedding 文件和现有 Pyannote segmentation 文件随 APK 资源打包；模型选择只影响 `SpeakerEmbeddingExtractor` 及 `OfflineSpeakerDiarization` 的 embedding 文件。
- 网页通过测试 APK 的本地模型选择路由切换模型，选择不持久化；APK 重启后默认使用 `ERes2Net-base`，不新增生产端或控制端远端配置。
- 模型加载失败时保留明确的未就绪状态并禁用注册/测试按钮；切换过程串行执行，不允许与注册、单段或多人测试并发。

实现状态：已完成。三个 ONNX 文件和 Pyannote 文件已纳入 APK 资源复制任务；原生页面与内置网页共用模型 ID，HTTP 状态/切换接口和注册、测试结果均返回当前模型信息。

真机验证（SM-N9500，Android 9/API 28，arm64-v8a）：三个模型均可加载并完成注册、单段声纹匹配和 ASR；切换模型后注册库清空，未发现 OOM、崩溃或 native 模型加载错误。

## 声纹 FP32/INT8 A/B 对比（2026-09-12）

在三个 Sherpa embedding 模型之外增加精度选择，用于在同一 APK、同一设备和同一批语音上比较 FP32 与 INT8，不替换已经验证的 FP32 模型。

- 原生页面和内置网页各增加 FP32/INT8 精度选择，默认 FP32；模型和精度组合共同标识一个运行变体。
- 三个 INT8 文件使用离线静态校准生成，校准输入使用 Sherpa 声纹前端的 80 维 Fbank 特征；ASR、Pyannote、流式 ASR 和 GTCRN 降噪模型不量化。
- 同一时间只加载一个模型变体。切换模型或精度时串行释放旧 native 引擎并清空注册库，避免不同精度的 embedding 混用；每个变体从独立的空注册库开始，阈值初始仍为 Sherpa 默认值 0.5。
- 注册、单段、多段和快速多段结果均返回模型、精度、变体 ID、embedding 维度和耗时，便于 A/B 记录；选择不持久化，重启回到 FP32/base。
- 量化文件必须先通过主机 ONNX Runtime 推理校验，再安装 APK 到 SM-N9500 真机，使用 `妲已妈妈.wav` 注册为 `妲己妈妈`，使用 `你好，小爱.wav`、`你好啊你好啊你好啊.wav` 和 `zh.wav` 做单段测试。
- 量化只作为测试选项；若真机出现加载失败、速度不升反降或匹配分数明显退化，保留 FP32 作为默认，不影响生产声纹链路。

实现状态：已完成。三个 `_int8.onnx` 文件由静态校准脚本生成并随 Debug APK 打包；原生页面和内置网页均可切换六种模型/精度组合。主机校验四段音频的 FP32/INT8 embedding 余弦为 `0.967–0.990`。

真机验证（SM-N9500，Android 9/API 28，arm64-v8a）：三个 INT8 变体均可加载、注册和单段推理，无崩溃、OOM 或 native 加载错误。以 `妲已妈妈.wav` 注册为“妲己妈妈”后，`你好，小爱.wav` 和 `你好啊你好啊你好啊.wav` 在三个 INT8 模型上均匹配成功；`zh.wav` 三个模型均低于默认 `0.5` 阈值，未匹配。对应 FP32/INT8 分数变化小，暂不调整阈值，INT8 仅保留为 A/B 测试选项。

## INT8 分段速度与行为复测（2026-09-12）

使用普通 `SHERPA_MULTI`、`speakerCount=AUTO`、不启用降噪，仍以 `妲已妈妈.wav` 注册“妲己妈妈”，在每个模型/精度组合启动独立 APK 进程后测试三段输入。

- Pyannote 分段耗时主要在 `0.23–0.31s`，INT8 与 FP32 基本相当，因为分段模型没有量化或替换；速度收益主要来自分段后的 embedding 提取。
- 三段测试的平均 `embeddingMs`：base INT8/FP32 为 `563/1299ms`，large 为 `1833/4465ms`，V2 为 `1653/3041ms`。本轮 INT8 分别约减少 `57%`、`59%`、`46%`；总 `elapsedMs` 还受到分段 ASR 波动影响，不能只用总耗时判断量化收益。
- `你好，小爱.wav` 三个模型/精度均得到 1 段并匹配注册人；重复“你好啊”时 base/large 得到 2 段，V2 得到 1 段，属于模型分段行为差异；`zh.wav` 均得到 1 段且未越过 0.5 阈值。
- 连续运行多个大模型后直接切换到 large FP32 曾触发 Android `lmkd` 的 `signal 9` 回收 APK 进程；单模型/精度独立重启后均可完成测试。因此性能结论采用独立进程数据，长时间连续切换仍需避免或继续做 native 内存释放优化。

## 测试 APK 临时精简声纹模型包（2026-09-20）

为优先验证录音链路并降低测试 APK 体积，当前测试 APK 暂时只打包 `ERes2Net-base FP32` 声纹 embedding 模型；多人分段依赖的 `pyannote_segmentation_3_0_int8.onnx` 保留。large、V2 及所有 INT8 embedding 文件暂不进入 APK，原生页面、内置网页和 HTTP 状态接口也只暴露 base + FP32，避免用户选择到不存在的资源。

该限制只作用于独立 ASR 测试 APK，不改变正式 APK、服务器模型目录和历史模型对比设计。后续恢复其他模型时，需要同步恢复 Gradle 白名单、VoiceprintModel/VoiceprintPrecision 枚举、页面选项、HTTP 校验和对应回归测试。

验收状态（2026-09-21）：APK 资源清单、构建、Android JVM 单测、Node 契约测试和 SM-N9500 覆盖安装均通过；主内置麦克风 ID 10 及蓝牙设备 ID 922 均已完成现场录音验证。

## 测试 APK 体积诊断（2026-09-21）

当前 Debug APK 约 `503,520,540 bytes`。模型和原生库是主要体积来源：SenseVoice ASR `model.int8.onnx` 约 239 MB，流式 ASR 模型约 51 MB，base FP32 声纹模型约 40 MB，ONNX/Sherpa 原生库约 32 MB，Pyannote 和降噪模型约 2 MB。

此前旧构建产物在资源表前存在约 223 MB 的全零填充区；重新执行完整 APK 构建后已消失。当前 APK 有效压缩内容和文件大小均约 280 MB，主要体积仍来自 ASR、流式 ASR、base FP32 声纹模型和原生库。

体积诊断验收（2026-09-21）：重新构建后的 APK 大小为 `280,663,752 bytes`，ZIP 完整性通过，已安装并上传到 LAN/WAN。

## 原生录音悬浮控制与音量折线图（2026-09-21）

原生录音页的录音切换按钮从滚动内容中移到根布局的底部悬浮层，始终显示在屏幕底部中央，并通过底部内边距避免遮挡滚动内容。录音线程复用每次约 100 ms 的 PCM 读取块，计算每块归一化峰值作为音量包络；录音结束后由原生自定义视图绘制完整时间轴折线，不新增录音通道、不影响蓝牙 SCO 路由和既有峰值/RMS 统计。

实现状态（2026-09-21）：已完成底部悬浮按钮、开始/结束文案切换、音量折线图、单元测试、APK 构建、SM-N9500 真机验证和 LAN/WAN 上传。
