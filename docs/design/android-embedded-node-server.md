...
APK 允许用户手动关闭显示界面后暂时离线；用户重新打开 APK 时，Node 子服务器必须在几秒内恢复连接。服务重启不能因为重复校验或复制大体积 Runtime 阻塞启动。

- 正常服务重启使用 `.runtime-version` 和关键启动文件的轻量检查，不对约 631 MB 的整个 Runtime 重复计算 SHA-256。
- Runtime 版本由构建时 Runtime、服务器包、依赖、证书和内置 Node 原生库的内容元数据生成；Node.js 版本不变但服务器代码变化时，APK 更新仍会触发一次必要安装。
