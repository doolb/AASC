...
- **选择文件发送**：媒体面板「发送 HTML」按钮选择本地 `.html` 文件，对话框内显示文件信息 + 滚动设置 + 去向：
...
- **循环播放**（独立开关，默认开启）：滚到底部后回顶重新开始（分页式/平滑均可循环）；不勾选则到底停止

### 4. 显示端渲染
- iframe 铺满整个屏幕，`pointer-events: none` 纯展示，不可交互
- 支持裁剪框设置（放大裁切）：裁剪框选中区域等比放大铺满屏幕，可叠加旋转（translate(cx,cy) → scale → 还原 → rotate → 居中）
- html 裁剪仅在适配模式为「裁剪（crop）」时生效，其他适配方式（适应/铺满/高度/宽度）还原默认铺满
- 上传端不预览 html 内容，裁剪预览区显示占位框（16:9 矩形模拟屏幕/iframe，90°/270° 切换 9:16 竖矩形并旋转，与显示端宽高互换一致）+ 裁剪框（基于占位框 rect，默认全屏，拖拽实时生效到显示端）
...
功能已全部实现（2026-08-15），包括：
- 服务端与上传端媒体类型识别（.html/.htm → html）
- 媒体库文件发送、工具栏粘贴发送（临时/保存媒体库）、裁剪框拖拽临时发送
- 显示端 iframe 全屏纯展示 + 分页式/平滑/循环滚动控制器
- 批量播放 html 项支持（默认分页 5 秒，暂停冻结/恢复继续）

实现细节见 `docs/task/2026-08-15_HTML媒体类型.md`。
