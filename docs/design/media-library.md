...
控制端发送远程媒体或播放列表时，为每项保留 `sourceNodeId`、`sourceLibraryId` 和 `sourcePath`。主服务器使用来源节点的注册地址生成直连地址，并保留同源代理回退地址。子服务器注册完成或地址变化后，主服务器可以按来源节点找到正在播放的媒体并重发既有正常播放消息；显示端不需要处理子服务器状态。
