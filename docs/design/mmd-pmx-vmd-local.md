...
- 独立补光开关、颜色、强度及经度/纬度方向，默认关闭，始终不投射阴影；
- 角色阴影开关，默认开启；
...
补光使用第三盏方向光，颜色默认白色、强度预设为 `1`、方向经度 `-45°`/纬度 `25°`，但默认关闭，旧版已保存设置缺少补光字段时仍关闭。开关关闭时实际光强为 `0`，开启后才使用滑块强度；任何状态下补光都不创建阴影贴图，不改变主光的阴影开关。补光方向随模型中心定位，切换模型和调节时即时更新；亮度预设保留当前补光自定义值，恢复默认会关闭补光。
