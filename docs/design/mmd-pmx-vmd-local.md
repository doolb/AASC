...
用户反馈快速旋转时布料仍会穿模。中心枢轴每帧按缓动移动，快速旋转使 type=0 运动学锚点大幅位移，原有约束牵引可能无法让动态布料在有限的物理子步内跟上。显示端灯光面板增加“旋转暂停物理阈值”，根据枢轴本帧实际 yaw/pitch 合成角位移除以帧时长得到度/秒，而非根据鼠标事件频率判断。默认 180 度/秒，可在 30–720 度/秒之间按 10 度/秒调节；超过阈值才暂停。
...
阈值沿用现有灯光设置对象及本地 `localStorage` 保存；预设保留当前值，“恢复默认”回到 180 度/秒，旧设置缺字段也取默认。此规则只作用于带 Ammo 物理的 PMX；无物理 PMX 和 VRM 不受影响。自动化验证覆盖状态切换与设置保存，真实模型快速 yaw/pitch、反向拖动和 Android WebView 的布料观感仍需现场验收。
