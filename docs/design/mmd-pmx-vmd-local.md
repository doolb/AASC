...
旋转矩阵分解可能得到 `0.9999999999999999` 等近似单位缩放。内置 MMDPhysics 的严格相等判断会错误进入临时脱离父节点的分支，使世界锚点丢失角色中心枢轴变换并拉乱布料。按用户要求，三个缩放分量与 1 的差值允许 `0.001`，仅超出时执行既有尺度处理。保留动态布料线速度、角速度及锚点约束牵引方式。

本地补丁位于内置 Three.js r160 `MMDPhysics.js`，升级 vendor 时须保留或重新验证。连续 yaw/pitch/组合缓动与缩放容差回归通过；真实模型和 Android 显示端仍待现场验证。
...
低于快速旋转阈值的普通缓动帧不调用 `MMDPhysics.reset()` 或清空速度、角速度、外力；快速旋转期间的暂停和恢复规则见第 14 节。不修改 PMX 刚体/关节、重力、风、VMD/IK/grant、物理频率设置，也不影响 VRM SpringBone。真实 PMX 仍须现场检查 yaw、pitch、反向拖动、30/65/90 Hz 下的布料穿模及自然收敛。当前方案不在 Bullet 的多个固定子步间插值枢轴；若低渲染帧率或快转时仍乱，需重新评审 `docs/superpowers/specs/2026-09-23-pmx-rotation-substep-sync-design.md`，不能宣称已完全解决。
...
## 14. PMX 快速旋转时暂停布料物理（2026-09-23）

用户反馈快速旋转时布料仍会穿模。中心枢轴每帧按缓动移动，快速旋转使 type=0 运动学锚点大幅位移，原有约束牵引可能无法让动态布料在有限的物理子步内跟上。显示端灯光面板增加“旋转暂停物理阈值”，根据枢轴本帧实际 yaw/pitch 合成角位移除以帧时长得到度/秒，而非根据鼠标事件频率判断。默认 720 度/秒，可在 30–1440 度/秒之间按 10 度/秒调节；超过阈值才暂停。

暂停时仅关闭当前 PMX `MMDAnimationHelper` 的物理步进，VMD、IK、grant、角色旋转和画面渲染继续。速度降到阈值以内后，先在物理仍关闭的帧更新动画，再把刚体重置到当前骨骼世界姿态，并清除动态刚体旧线速度、角速度和外力；下一帧恢复物理。这会让布料在恢复点重新贴合当前姿态，可能有一次可见跳变，但避免旧世界位置和速度把布料拉穿角色。普通慢速旋转仍采用第 11 节的锚点牵引，不重置物理。

阈值沿用现有灯光设置对象及本地 `localStorage` 保存；预设保留当前值，“恢复默认”回到 720 度/秒，旧设置缺字段也取默认。已保存的 180 度/秒等自定义值不迁移，继续按保存值使用。此规则只作用于带 Ammo 物理的 PMX；无物理 PMX 和 VRM 不受影响。自动化验证覆盖状态切换与设置保存，真实模型快速 yaw/pitch、反向拖动和 Android WebView 的布料观感仍需现场验收。
