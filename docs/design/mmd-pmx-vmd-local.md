...
- 独立补光开关、颜色、强度及经度/纬度方向，默认关闭；阴影来源可在无阴影、主光、补光之间切换，任何时候最多一盏方向光投射阴影。
- 角色阴影开关，默认开启；
- PMX Toon 明暗开关，默认关闭；开启后主光和补光沿用 Toon 渐变；
- 两组仅作用于 PMX 的边缘光，各自可调开关、颜色、强度和经纬度方向，默认关闭；
- PMX 物理目标步进频率，范围 30–90 Hz、步长 5 Hz、默认 65 Hz；
- 默认、柔和、明亮预设和“恢复默认”。

灯光/物理设置通过既有 `DisplayMmd.setLighting()` 流程传给当前 runtime，规范化值保存到当前显示端浏览器 `localStorage`，显示端重新打开时恢复；不发送 WebSocket，不写入服务端配置，不进入 Offline APK。灯光值更新已创建的 Three.js light 对象，PMX Toon 模式与两组边缘光另更新已加载 PMX 材质的 shader uniform。物理频率只作用于 PMX：将 `MMDPhysics.unitStep` 设为 `1 / physicsFps`，当前模型即时生效，新建 helper 也使用相同值；不重新加载模型、纹理或 VMD。VRM 忽略该设置。物理频率是目标步进频率，实际步数仍受渲染帧率及 `maxStepNum=3` 限制；调高频率会增加物理计算开销，模型间的稳定性可能不同。

补光使用第三盏方向光，颜色默认白色、强度预设为 `1`、方向经度 `-45°`/纬度 `25°`，但默认关闭，旧版已保存设置缺少补光字段时仍关闭。开关关闭时实际光强为 `0`，开启后才使用滑块强度；阴影来源三选一，默认主光，选补光但补光关闭时实际无阴影。两盏灯各保留阴影相机参数，运行时只启用选中灯的阴影贴图，避免双阴影开销。旧布尔 `shadowEnabled=false` 映射为“无阴影”，旧 true/缺失映射为“主光源阴影”，新 `shadowSource` 优先。补光方向随模型中心定位，切换模型和调节时即时更新；亮度预设保留当前补光自定义值，恢复默认会关闭补光并选择主光阴影。

阴影来源由显示端统一灯光状态传递到 PMX runtime；VRM 暂不实现补光自身阴影，选择该模式时禁用 VRM 主光阴影，避免显示错误来源。正式显示端的 VRM 补光阴影如需与 PMX 完全一致，后续单独实现与测试。

PMX 的 Toon 模式使用原有渐变贴图；默认关闭“PMX Toon 明暗”，让 PMX 直射光使用法线点积的非负值，因此背向主光的表面不接收主光，但若补光从另一面照入仍可变亮。此开关同时作用主光和补光，不改 ACES 色调映射、环境光、AO、阴影、VRM 材质或模型文件。运行时只为已加载的 PMX 材质实例注入模式参数，切换时更新参数而不重建模型；旧浏览器配置缺失该字段时默认使用普通光照，明确保存过的 Toon 开启状态保留，灯光预设保留模式，恢复默认关闭 Toon。米娅 PMX 的 52 个材质均使用内置纯白 `toon00.bmp`，故旧 Toon 模式下主光对背光面的直射响应不为零，普通模式正是为此提供可选修正。

两组边缘光不是新的 Three.js DirectionalLight：其颜色只在 PMX 当前可见表面接近视线切线、且法线朝向对应边缘光方向时叠加，正面中心不增加漫反射，高光或角色阴影亦不受其影响。两组均默认关闭，默认方向分别为经度 `-130°` 和 `130°`、纬度 `25°`，颜色分别为 `#8acbff` 和 `#ffb6d9`，预设强度均为 `1`；各自可独立启停与调节，关闭时实际强度为 `0`。边缘光不影响 VRM；旧本地设置缺少这两组参数时按关闭处理，柔和/明亮预设保留自定义边缘光，恢复默认关闭。材质加载时仅注入一次 shader 参数，设置变更只更新 uniform，不重载模型或动作。
...
PMX 材质的“环境色”与场景环境光并非同一设置。Three.js `MMDLoader` 会将 PMX 环境色映射成材质 `emissive`，导致场景灯都为 0 时人物仍呈灰色。按当前显示规则，所有 PMX 在加载完成、尚未入场时清零 `MMDToonMaterial.emissive`，始终忽略 PMX 环境色；保留 diffuse、纹理、透明度和 toon 阶调，不修改 vendored 加载器或 VRM 材质。此规则对本地和同源代理加载的 PMX 一致，普通光照下角色会比以前略暗；环境光、主光与补光实际强度都为 0 时模型不再因环境色自行显灰。
...
PMX runtime 先把原场景渲染到带深度纹理的透明色彩目标，再按面板选择的半分辨率或全分辨率计算 AO。着色器根据深度重建可见表面位置和法线，采样周围深度估计遮蔽。AO 图再经过水平与垂直两次按视空间深度保边的模糊，减轻随机采样在衣服和头发上的颗粒噪点；合成时继续按深度边缘抑制背景/前景串色，仅调整原色彩的 RGB，保留 alpha，使角色和透明舞台的分层关系保持一致。
...
为解决 AO 在高强度下的颗粒和模型移动时的闪烁，面板增加每像素 `12 / 24 / 32` 次采样选择，默认 24；旧浏览器设置缺少该字段时也用 24，合法自定义值随其他灯光设置保存。采样结果按实际采样数归一化，运行时只更新 shader uniform；两次保边模糊扩展到每轴左右各 3 个邻点，并用深度差平方衰减权重，在同一表面扩大降噪范围，尽量保留衣袖和发丝深度边缘。采样和空间模糊可降低颗粒与部分闪烁，但没有跨帧累积，不能保证动作期间完全无闪烁；若现场仍明显，需另行设计历史失效与重投影，避免鬼影。32 次或全分辨率将增加手机 GPU 成本，需对比帧率后选择。

“AO 模糊轮数”可选 0–3，默认 1；每轮独立设置 1–5 AO 像素的水平/垂直保边模糊半径，默认均为 3。选择 0 轮时直接合成原始 AO；选择多轮时复用现有 AO 与临时目标交替渲染，不重新分配贴图。面板仅显示当前轮数对应的半径控件，参数即时生效并随现有本地灯光配置保存；旧设置缺字段时回退默认值。这里的模糊半径以 AO 目标像素计，不同于角色高度百分比定义的世界空间 AO 采样半径。多轮和大半径会增加纹理读取与绘制通道，目标 Android WebView 的观感/帧率仍须现场验收；网页贴图瘦身另行处理。

共用显示端代码已发布到外网 HTTPS MMD AR 测试网页，包括后续新增的 0–3 轮独立半径模糊控件，供远端手机观察效果；正式 Offline 服务更新清单和 APK 不随此测试页发布。网页功能测试与目标手机画质、帧率验收仍待现场进行。
