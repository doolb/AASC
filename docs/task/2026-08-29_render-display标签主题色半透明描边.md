...
`render-display` 进度条外的设备名、`M`、`GPU` 和 `VRAM` 标签，以及显示端时间、媒体名在复杂媒体背景上对比度不足，需要保持固定文字效果，并增加右下方 1px、无模糊的主题色斜向投影。

## Design 需求

- 进度条外标签、时间文本和媒体名使用固定白色文字和固定黑色阴影。
- 增加右下方 1px、无模糊的当前主题强调色斜向投影。
...
2. 检查三类文字保留固定黑色阴影。
3. 检查三类文字具有右下方 1px 主题色无模糊斜向投影和半透明 `color-mix()` 实现。
...
- 已确认：进度条外文字、时间和媒体名统一保留固定黑色阴影，并增加右下方 1px、无模糊的半透明主题色斜向投影。
