...
- OCR 任务支持 `shortSide` 目标短边上限；YOLO 不增加缩放配置。
...
7. OCR 使用 `0/736/512/384` 时，显示端按比例缩小并将结果坐标映射回原图；YOLO 请求协议保持不变。
...
- 任务面板和 OCR 内置任务新增 `shortSide`，提供自动 `0`、`736`、`512`、`384` 四档；服务器 `/api/vision/ocr` 仅校验并通过 `visionOcr` 转发，YOLO 协议不变。
- 正式 APK 新增带尺寸的 NativeBridge 方法；RapidOCR 按比例只缩小工作图，返回原图尺寸和映射回原图的文字框坐标。旧的两参数 OCR 方法继续保留，`shortSide=0` 兼容旧调用。
- 真机 `192.168.1.6:5555`、显示端 `display-umwo5k6l` 使用 `/mnt/tmp` 现有 4 张截图完成 16/16 成功测试。平均 OCR 本地耗时：自动 `30,528ms`、736 `19,622ms`、512 `15,045ms`、384 `11,416ms`；平均文字框：`15.5/15.0/14.75/12.5`。全部为“单小核（核心 0）”。
- 由于无标注集，不能把框数变化直接解释为准确率变化；384 档对小字号文字有漏检风险，业务默认仍为自动，建议速度与识别平衡时选择 512 或 736。
