...
- 设备名、`M`、`GPU`、`VRAM` 等进度条外标签使用固定白色文字和固定黑色阴影，并在文字右下方使用 1px、无模糊的当前主题 `accent-color` 斜向投影；投影优先使用约 65% 透明度的 `color-mix(in srgb, accent-color 65%, transparent)`，同时保留纯色回退，不使用文字轮廓或元素盒子内阴影
