...
- GPU 第二行使用小幅动态占位：`spacerWidth = max(0, deviceNameW - lblW - rowGap)`；在当前布局参数下常规模式为 `42px`、紧凑模式为 `34px`。GPU 进度条与 CPU 进度条对齐，VRAM 进度条与 MEM 进度条对齐；GPU 与 VRAM 条之间保留布局定义的行内间距
