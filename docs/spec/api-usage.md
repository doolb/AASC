...
common.sh:
    ...
    CURL_ARGS = silent + show-error + connect-timeout + max-time
    if AASC_INSECURE != 0: append --insecure
    append --noproxy 127.0.0.1,localhost,::1

    api_request(method, path, curl_arguments):
        url = BASE_URL + path unless path is an absolute URL
        temporaryFile = mktemp
        run curl with --output temporaryFile and --write-out HTTP status
        if transport fails:
    ...
    json_quote(text):
        use jq when available, otherwise node JSON.stringify
```

## 通用调用器伪代码

```text
api-request.sh METHOD PATH [--json JSON] [--form key=value] [--file key=FILE]
    parse --help without network access
    validate method and path
    convert --json to Content-Type application/json + request body
    convert --form and --file to curl multipart arguments
    if method/path is dangerous and --confirm is absent:
        print confirmation error to stderr
        return nonzero
    call common.api_request
```

```text
common.sh:
    本机回环地址不经过环境代理，远程 AASC_URL 仍按 curl 的普通代理规则处理
...
```text
静态契约测试：
    assert all user-facing scripts exist and have Bash shebang
    assert all scripts source common.sh
    assert help paths do not invoke curl
...
本机命令测试：
    ...
    verify dangerous interface without confirmation does not invoke curl

真实验证记录：
    run-all.sh 和 npm run api:test 的只读探针全部成功
