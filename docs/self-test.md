...
| PMX 快转暂停物理 | 将“旋转暂停物理阈值”设为 180°/秒，快速拖动 yaw/pitch 和反向拖动，确认动作与角色旋转继续、布料不再被快速锚点牵引穿模；减速后观察布料重新贴合及继续摆动 | 已加载带布料刚体的 PMX；对比 30、180、720°/秒，留意恢复瞬间跳变 |
| PMX 旋转阈值保存 | 调节阈值后刷新显示页确认恢复；切换柔和/明亮灯光预设应保留该值，“恢复默认”回到 180°/秒 | 浏览器允许 localStorage |
