...
- ⏳待处理 [2026-09-01] 将 Termux 服务器试运行整理为正式 Android 节点
  - 当前 `~/aasc-server-test` 已能在 Termux 以 runit 服务运行，使用 8081 端口；服务器发布包现由 `npm run build:server-package` 显式生成，后续需要让子服务器主动向主服务器注册并发送心跳，再增加认证、能力声明、媒体同步和 Android 重启自动恢复。
