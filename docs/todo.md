...
- 🔄进行中 [2026-09-19] 显示端聊天与 VRM/MMD 同位分层
  ...
  - 🔄待现场验证 [2026-09-23] PMX 旋转缓动布料扰动：已修正单位缩放浮点误差导致脱离枢轴的问题，容差为 0.001；检查真实模型 yaw/pitch、组合旋转、反向拖动、停转惯性及不同物理频率的视觉效果。
  - 当前试验任务：`docs/task/20260923_PMX旋转锚点牵引布料试验.md`；若仍失稳，重新评审备选设计 `docs/superpowers/specs/2026-09-23-pmx-rotation-substep-sync-design.md`，不得直接恢复整批刚体传送。
