...
- ⏳待处理 [2026-09-01] 将 Termux 服务器试运行整理为正式 Android 节点
  - 当前 `~/aasc-server-test` 已能在 Termux 以 runit 服务运行，使用 8081 端口；后续需要让子服务器主动向主服务器注册并发送心跳，再增加认证、能力声明、媒体同步和 Android 重启自动恢复。
...
- ⏳进行中 [2026-09-05] 将子服务器改为主动连接 AASC 主服务器
  - 已确认使用固定主服务器 `https://192.168.1.39:8081/` 和 WebSocket `/server` 长连接；主服务器通过连接处理注册、心跳、远程索引、服务节点任务和热更新命令。
  - 暂不实现权限认证、自动发现、最近服务器算法和媒体文件同步。
  - 设计文档：`docs/superpowers/specs/2026-09-05-aasc-subserver-active-connection-design.md`。
