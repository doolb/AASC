...
- 🔄进行中 [2026-09-22] 将已在本地验证的 PMX/VMD 资源复用到公网静态资源并切换默认发布配置
  - 本地阶段已完成：`res/models/mmd/` 目录、manifest 白名单、PMX/VMD 浏览器加载和默认循环动作播放；未处理 Offline APK。
  - 本地显示端现已等待 PMX 纹理和 VMD 完整准备后再提交模型，避免加载过程逐材质显示和画面穿帮。
  - 本地显示端已增加时间区域下方的灯光按钮、详细灯光面板、预设/恢复默认和浏览器本地保存；角色阴影默认开启并可关闭，PMX/VRM 人物同时支持投射和接收自阴影；未接远端配置、Offline APK 或外网发布。
  - 阴影质量已优化为 `1024×1024` 动态包围盒阴影相机；已完成空白区域双轴拖动旋转，只有按下时未命中角色才旋转：水平转身、纵向俯仰限制在上下各 45°，命中角色仍保留既有轻点互动。
  - 待后续单独完成：资源授权复核、版本化外网目录、SHA-256/清单发布、WAN 回退验证和默认配置切换；在完成前不上传外网。
