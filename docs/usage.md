...
显示端打开“角色”并等待模型加载后，可点击右上角“定位”。点击“拍照校准”会打开独立弹窗：先在实时画面中拍照，再在同一位置拖动四个角点选择细节丰富的基准图区域，最后保存。选择定位图并点击“开始定位”后，摄像头画面显示在角色下方；识别到基准图时，角色脚底会跟随基准图中心。点击“停止定位”释放摄像头并恢复普通角色布局。自然图片应有文字或明显纹理；低光、反光、遮挡和快速转动可能使定位暂时丢失。图片保存在当前显示端浏览器本地，不上传服务端。
