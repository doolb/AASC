...
显示端打开“角色”后，点击右上角“灯光”可调整环境光、主光和角色阴影。PMX 角色还有默认开启的“PMX 环境遮蔽 AO”开关：开启后衣服、头发和身体相邻处会有更明显的接触暗部，关闭可减轻图形设备负担。开关下方可调整 AO 颜色（默认 `#931231`）、强度（`0–2`，默认 `0.6`）及半径（角色高度的 `1–20%`，默认 `6%`）；颗粒噪点由保边模糊减轻，半径过大仍可能使暗部过宽。PMX 角色还可设置“PMX 物理频率”，范围为 30–90 Hz，默认 65 Hz；频率是物理目标步进值，实际计算还受页面帧率及每帧最多 3 个物理子步限制，调高频率会增加计算量。“旋转暂停物理阈值”默认为 180°/秒，可在 30–720°/秒调节；角色旋转速度超过该值时，布料物理暂时停止，减速后重新贴合当前姿态并恢复。上述设置立即应用并保存在当前浏览器，刷新页面后恢复。VRM 角色不使用 PMX AO、物理频率或旋转阈值。
