...
- 控制端可配置的服务端/显示端运行参数，优先通过现有 WebSocket 远端配置流程下发。
- 标准流程为：控制端发送配置消息 → 服务端校验并通过 `config.set` 持久化 → 服务端广播规范化后的权威值 → 目标端连接时补发当前值。
- 同一配置不新增独立的 HTTP GET/POST 读取或保存接口；已有业务查询接口保持原职责。
- 配置消息必须包含 `type`，服务端必须处理非法值和范围，并向控制端回传最终生效值。
- 目标端在断线或尚未收到配置时使用明确默认值，重连后以服务端补发值为准。

## 使用文档同步规则

- 纯使用说明、操作示例或当前行为解释的修改，只更新 `docs/usage/` 对应文档及 `docs/usage.md` 入口，不新增 design、spec 或 task 文件。
